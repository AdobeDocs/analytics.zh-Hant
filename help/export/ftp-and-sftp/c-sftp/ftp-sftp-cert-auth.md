---
description: 若要不使用密碼連線至 FTP 帳戶，必須同時使用 SFTP 連線和替代驗證方法。 這需要用到稱為公開金鑰和私密金鑰的兩個檔案組合 (一個檔案存在 FTP 帳戶上，另一個檔案存在您的電腦上)。
keywords: ftp;sftp
...
若要不使用密碼連線至 FTP 帳戶，必須同時使用 SFTP 連線和替代驗證方法。 這需要用到稱為公開金鑰和私密金鑰的兩個檔案組合 (一個檔案存在 FTP 帳戶上，另一個檔案存在您的電腦上)。
