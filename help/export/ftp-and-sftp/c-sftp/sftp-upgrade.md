...
2022年夏天，Adobe Analytics將升級其安全檔案傳輸協定 [SFTP] 為檔案傳輸提供改進的安全性。 在這次變更後，我們將不再支援某些 SFTP 用戶端設定。這僅影響使用SFTP發送到Adobe Analytics或從其檢索到的資料。 FTP 通訊協定將不受影響。為避免服務中斷，請確保您的 SFTP 用戶端 (程式碼、工具、服務) 將符合以下詳述的變更。
