...
* 目的地支援單因素 (PKI) 驗證。如果有雙因素問題，報表傳遞會失敗。請確保未將伺服器配置為嘗試雙因素身份驗證。 Adobe Analytics 要求在登入中只能使用金鑰，不能有其他項目。
* Adobe 支援 SSHv2 加密，並可回復為 SSHv1 (僅限 RSA 金鑰)。

若要透過SFTP成功傳送Data Warehouse請求：

1. 請貴組織的受支援使用者聯絡客戶服務，取得 `authorized_keys` 檔案。
1. 取得該檔案後，使用用於資料倉庫請求的相同認證登入 FTP 站台。
1. 在根目錄瀏覽至名為 `.ssh` 的資料夾 (如果該資料夾不存在，請建立一個)，然後將 `authorized_keys` 檔案置於該處。

1. 前往資料倉庫請求管理員。視需要設定請求，然後按一下「**[!UICONTROL 進階傳送選項]**」。

1. 在快顯視窗中按一下&#x200B;**[!UICONTROL 「FTP」]**，接著指定 FTP 站台 (包括 `sftp://` 通訊協定，例如 `sftp://ftp.omniture.com`) 經由連接埠 22。
