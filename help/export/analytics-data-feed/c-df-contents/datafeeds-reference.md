...
| `country` | 數值ID表示 `country.tsv` 查閱。 用於「Reports &amp; Analytics」的「頂級網域」報表。 | 不帶正負號的 smallint |
...
| `geo_city` | 根據IP的點擊來源城市名稱。 用於[城市](/help/components/dimensions/cities.md)維度。 | char(32) |
| `geo_country` | 點擊來源國家/地區的縮寫，根據IP。 用於[國家/地區](/help/components/dimensions/countries.md)維度。 | char(4) |
| `geo_dma` | 點擊來源之人口統計區域的數值ID（根據IP）。 用於 [美國 DMA](/help/components/dimensions/us-dma.md) 維度。 | 不帶正負號的 int |
| `geo_region` | 點擊來源的州或地區名稱（根據IP）。 用於[地區](/help/components/dimensions/regions.md)維度。 | char(32) |
