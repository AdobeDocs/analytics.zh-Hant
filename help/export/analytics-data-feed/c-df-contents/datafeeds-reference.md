...
此表格先前的更新專案可在此頁面的 [在GitHub上認可歷史記錄](https://github.com/AdobeDocs/analytics.en/commits/main/help/export/analytics-data-feed/c-df-contents/datafeeds-reference.md).
...
| **`carrier`** | Adobe Advertising 整合變數。指定行動電信業者。的鍵值 `carrier.tsv` [動態查詢](dynamic-lookups.md). | varchar(100) |
...
| **`click_action`** | 已不再使用。在舊版ClickMap工具中點選的連結位址。 | varchar(100) |
| **`click_action_type`** | 已不再使用。舊版ClickMap工具的連結型別。<br>0：HREF URL<br>1：自訂 ID<br>2：JavaScript onClick 事件<br>3：表單元素 | 不帶正負號的 tinyint |
...
| **`cust_hit_time_gmt`** | 僅限啟用時間戳記的報告套裝。隨點選傳送的時間戳記(以UNIX®時間為基礎)。 | int |
...
| **`first_hit_time_gmt`** | 訪客第一次點選的時間戳記(以UNIX®時間表示)。 | int |
...
| **`geo_zip`** | 根據IP的點選來源郵遞區號。 協助填入[郵遞區號](/help/components/dimensions/zip-code.md)維度。另請參閱`zip`。 | varchar(16) |
| **`hier1 - hier5`** | 由階層變數使用。包含使用分隔符號的值清單。在報告套裝設定下選擇分隔符號。 | varchar(255) |
| **`hit_source`** | 指出點選的來源。 點擊來源 1、2 和 6 需計費。<br>1：沒有時間戳記的標準影像要求<br>2：具有時間戳記的標準影像要求<br>3：具有時間戳記的即時資料來源上傳<br>4：未使用<br>5：通用資料來源上傳<br>6：完整處理資料來源上傳<br>7：TransactionID 資料來源上傳<br>8：不再使用；舊版 Adobe Advertising Cloud 資料來源 <br>9：已不再使用；Adobe Social 摘要量度<br>10：使用 Audience Manager 伺服器端轉送 | 不帶正負號的 tinyint |
| **`hit_time_gmt`** | 點選Adobe資料收集伺服器收到點選的時間戳記(根據UNIX®時間)。 | int |
| **`hitid_high`** | 搭配使用 `hitid_low` 以識別點選。 | 不帶正負號的 bigint |
| **`hitid_low`** | 搭配使用 `hitid_high` 以識別點選。 | 不帶正負號的 bigint |
...
| **`last_hit_time_gmt`** | 先前點選的時間戳記(UNIX®時間)。 用於計算[和上次造訪間隔天數](/help/components/dimensions/days-since-last-visit.md)維度。 | int |
...
| **`mobile_id`** | 如果使用者使用行動裝置，則為裝置的數值 ID。的鍵值 `mobile_attributes.tsv` [動態查詢](dynamic-lookups.md). | int |
...
| **`mobilehourofday`** | 定義應用程式在一天中的哪個小時啟動。 請依照 24 小時數字格式。 | varchar(255) |
...
| **`mvvar1_instances`** - `mvvar3_instances` | 在目前點擊上設定的清單變數值。將原始分隔符號取代為 `--**--`. 沒有 `post` 欄。 | 文字 |
| **`namespace`** | 未使用。屬於已報廢功能。 | varchar(50) |
| **`new_visit`** | 此旗標可確定目前的點擊是否為新造訪。造訪閒置 30 分鐘後，由 Adobe 伺服器設定。 | 不帶正負號的 tinyint |
| **`os`** | 表示訪客的作業系統的數值 ID。依據`user_agent`欄而定。的鍵值 `operating_system.tsv` 標準查詢和 `operating_system_type.tsv` [動態查詢](dynamic-lookups.md). | 不帶正負號的 int |
...
| **`search_page_num`** | 由[所有搜尋頁面排名](/help/components/dimensions/all-search-page-rank.md)維度使用。指出在使用者點進您的網站之前，您的網站出現在搜尋結果的哪個頁面。 | 不帶正負號的 smallint |
...
| **`visid_high`** | 搭配使用 `visid_low` 以唯一識別訪客。 | 不帶正負號的 bigint |
| **`visid_low`** | 搭配使用 `visid_high` 以唯一識別訪客。 | 不帶正負號的 bigint |
| **`visid_new`** | 用於識別點擊是否包含新產生訪客 ID 的旗標。 | char(1) |
| **`visid_timestamp`** | 如果是新產生訪客ID，則會提供產生訪客ID時的時間戳記(UNIX®時間)。 | int |
...
| **`visit_start_time_gmt`** | 造訪之第一次點選的時間戳記(UNIX®時間)。 | int |
