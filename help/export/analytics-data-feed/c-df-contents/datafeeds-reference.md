...
| `mc_audiences` | 訪客所屬的 Audience Manager 區段 ID 清單。此 `post_mc_audiences` 欄將分隔字元變更為 `--**--`. | 文字 |
...
| `mvvar1` - `mvvar3` | 清單變數值。根據實作包含使用分隔符號的自訂值清單。此 `post_mvvar1` - `post_mvvar3` 欄會以取代原始分隔字元 `--**--`. | 文字 |
