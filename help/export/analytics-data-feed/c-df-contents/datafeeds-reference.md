...
| `cookies` | 用於「Cookie 支援」維度的變數。<br>Y:啟<br>用N:禁<br>用U:未知 | char(1) |
...
| `mobileappperformancecrashid` | 用於Apteligent資料連接器。 Apteligent中使用的當機ID。 | varchar(255) |
...
| `mobiledeeplinkid` | 從上下文資料變數a.<span>deeplink</span>.id收集。 用於贏取報表中，作為行動贏取連結的識別碼。 | varchar(255) |
...
| `mobilemessagepushoptin` | 從上下文資料變數a.push.optin收集。 當使用者選擇推送訊息時，設為「true」;否則，值為&quot;false&quot;。 | varchar(255) |
...
| `mobilerelaunchcampaignsource` | Mobile services啟動來源 | varchar(255) |
...
| `videoaudioartist` | 視訊音訊藝術家 | varchar(255) |
