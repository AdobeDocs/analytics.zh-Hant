...
* 資料饋送應發送的頻率（如果您選擇包括晚到點擊，則包括回望窗口）
...
1. 選取&#x200B;[!UICONTROL **建立資料摘要**]。
   ...
   顯示的頁面具有以下類別： [!UICONTROL **詳細資訊**]、[!UICONTROL **資料格式**]、[!UICONTROL **資料結構**]、[!UICONTROL **計畫**]&#x200B;和&#x200B;[!UICONTROL **目標**]。

   ![新資料饋送頁](assets/data-feed-new.png)

1. 在&#x200B;[!UICONTROL **詳細資訊**]&#x200B;部分，填寫以下欄位：
...
1. 在&#x200B;[!UICONTROL **資料格式**]&#x200B;區段中，指定下列資訊：
   ...
   | [!UICONTROL **資訊清單**] | 選擇是否將清單檔案包括在每個資料饋送傳遞中。 <p>您可以從以下選項中進行選擇：</p><ul><li>**[!UICONTROL 清單檔案]**：包含資料饋送中包含的每個檔案的資訊。</li><li>**[!UICONTROL 完成檔案（舊版）]**：指示資料饋送已成功完成。 不包括其他資訊。 此選項適用於原本使用此選項但需要重新處理的現有摘要。 它僅適用於以單一套件傳送資料摘要資料時。 </li><li>**[!UICONTROL 無]**：不包含任何檔案</li></ul> |
   ...
   | [!UICONTROL **替換作業系統字串**] | 在收集資料時，某些字元（如新行）可能會導致問題。 選擇此選項可將這些字元從源檔案中刪除。<p>此選項會偵測以下內嵌於客戶資料中的字串序列，並以空格取代：</p> <ul><li>**Windows：** CRLF、CR或TAB</li><li>**Mac和Linux：** \n、\r或\t</li></ul> |
   | [!UICONTROL **啟用動態查找**] | 動態查找允許您在資料源中接收其他不可用的查找檔案。 此設定可讓以下查詢表格隨每個資料摘要檔案一起傳送：<ul><li> **運營商名稱**</li><li>**移動屬性**</li><li>**作業系統類型**</li></ul><p>有關詳細資訊，請參閱[動態查找](/help/export/analytics-data-feed/c-df-contents/dynamic-lookups.md)。</p> |
   | **允許延遲到達的點擊** | 歷史資料可以在資料饋送作業完成給定小時或一天的處理後到達，例如通過時間戳命中或資料源。<p>選取此選項，可在設定的報告頻率內 (通常是每日或每小時)，納入資料摘要工作完成資料處理之後抵達的資料。啟用此選項，則每次資料摘要處理資料時，都會查看任何送達的延遲點擊，並將其整批放進下一個傳送的資料摘要檔案中。</p><p>有關詳細資訊，請參閱[延遲到達的點擊](/help/export/analytics-data-feed/c-df-contents/late-arriving-hits.md)。</p> |
...
1. 在&#x200B;[!UICONTROL **資料結構**]&#x200B;區段的&#x200B;**[!UICONTROL 報表套裝]**&#x200B;欄位中，選取包含您要匯出之資料的來源報表套裝。 <p>選取報表套裝時，請考量下列事項：</p> <ul><li>如果為同一報表套件建立了多個資料饋送，則每個資料饋送必須具有不同的列定義。</li><li>只有來源報表套裝支援資料摘要，虛擬報表套裝並不受支援。</li><li>可用列的清單取決於所選報告套件所屬的登錄公司。 如果您變更報表套裝，可用欄的清單可能會變更。 </li></ul>
...
1. 請使用下列其中一種或兩種方法來決定要將哪些資料欄包含在摘要中：
   ...
   * **個別新增欄：**&#x200B;在左側的&#x200B;**[!UICONTROL 可用]**&#x200B;區段中，選取您要包含的任何欄，然後選取&#x200B;**[!UICONTROL 包含]**。 Adobe Analytics中的所有資料欄皆可使用。 您可以按住&#x200B;**[!UICONTROL Shift]**&#x200B;或按住&#x200B;**[!UICONTROL Command]** （在macOS上）或&#x200B;**[!UICONTROL Ctrl]** （在Windows上）來選取多個欄。 按一下&#x200B;**[!UICONTROL 「全部新增」]**，將所有欄加入資料摘要中。
     ...
     您添加的列顯示在右側的&#x200B;**[!UICONTROL 已包括]**&#x200B;部分中。

   * **添加列模板：**&#x200B;在&#x200B;**[!UICONTROL 列模板]**&#x200B;欄位中，選擇要添加的列模板。 列模板是預定義的列組，預設情況下，Adobe提供多個列。

     模板中包含的所有列都顯示在右側的&#x200B;**[!UICONTROL 已包含]**&#x200B;部分。
...
1. 在&#x200B;[!UICONTROL **排程**]&#x200B;區段中，指定下列資訊：
   ...
   | 欄位 | 函數 |
   |---------|----------|
   | [!UICONTROL **頻率**] | 選取資料摘要的傳送頻率。 可用選項會根據您的報表套裝設定動態填入。 <p>下列為常用選項：</p><ul><li>**每日**：摘要包含一整天的資料，從報表套裝時區的午夜到午夜。 此選項用於回填或歷史資料，或用於繼續源。</li><li>**每小時**：源包含一小時的資料。 使用此選項繼續摘要。</li></ul><p>可能有15分鐘的導出頻率，但預設情況下不可用。 要使此選項在您的環境中可用，您必須首先與Adobe客戶服務部門聯繫，並請求將您的報告套件配置為支援15分鐘的導出。</p> |
   | [!UICONTROL **處理延遲**] | 選擇是否在處理資料饋送檔案之前等待給定時間。 延遲的作用在於，在行動裝置實作中讓離線裝置得以上線並傳送資料。在管理舊的已處理檔案時，它也可用於容納組織的伺服器端處理程序。多數情況下延遲並不會發生。摘要最多可延遲8小時（480分鐘），或如果選取自訂時間量（延遲9,999分鐘或約1週），延遲時間甚至更長。 |
...
1. 在&#x200B;[!UICONTROL **目標**]&#x200B;部分，配置要將資料發送到的目標。
   ...
   >
   >* 雲端帳戶與您的 Adob&#x200B;&#x200B;e Analytics 使用者帳戶有關聯。其他用戶無法使用或查看您配置的雲帳戶，除非您將這些帳戶提供給您組織中的所有用戶。
   >
   ...
   | 欄位 | 函數 |
   |---------|----------|
   | [!UICONTROL **帳戶**] | 進行下列一項：<ul><li>**使用現有帳戶：**&#x200B;選擇&#x200B;**[!UICONTROL 帳戶]**&#x200B;欄位旁邊的下拉菜單。 或者，開始鍵入帳戶名，然後從下拉菜單中選擇它。 <p>只有在您已設定帳戶，或帳戶與您所屬的某個組織共用時，您才可使用帳戶。</p></li><li>**建立新帳戶：**&#x200B;在&#x200B;**[!UICONTROL 帳戶]**&#x200B;欄位下選取&#x200B;**[!UICONTROL 新增]**。 如需有關如何設定帳戶的資訊，請參閱[設定雲端匯入和匯出帳戶](/help/components/locations/configure-import-accounts.md#configure-a-location-account)中的[設定位置帳戶](/help/components/locations/configure-import-accounts.md)。</li></ul> |
   | [!UICONTROL **位置**] | 進行下列一項：<ul><li>**使用現有的位置：**&#x200B;選取&#x200B;**[!UICONTROL 位置]**&#x200B;欄位旁的下拉式功能表。 或者，開始鍵入位置名稱，然後從下拉菜單中選擇它。</li><li>**建立新位置：**&#x200B;在&#x200B;**[!UICONTROL 位置]**&#x200B;欄位下選擇&#x200B;**[!UICONTROL 添加新]**。 有關如何配置位置的資訊，請參閱[配置雲導入和導出位置](/help/components/locations/configure-import-locations.md#configure-a-location)中的[配置位置](/help/components/locations/configure-import-locations.md)。</li></ul> |
...
管理模板時，您可以建立新模板、使用已建立的模板、複製模板、編輯模板和刪除模板。

**[!UICONTROL 管理]** > **[!UICONTROL 資料源]** > **[!UICONTROL 管理模板]**
...
1. 在&#x200B;**[!UICONTROL 模板名稱]**&#x200B;欄位中，指定模板的名稱。

1. 在左側的&#x200B;**[!UICONTROL 可用]**&#x200B;區段中，選取您要包含的任何欄，然後選取&#x200B;**[!UICONTROL 包含]**。 Adobe Analytics的所有可用資料列都可用。 您可以按住&#x200B;**[!UICONTROL Shift]**&#x200B;或按住&#x200B;**[!UICONTROL Command]** （在macOS上）或&#x200B;**[!UICONTROL Ctrl]** （在Windows上）來選取多個欄。 按一下&#x200B;**[!UICONTROL 「全部新增」]**，將所有欄加入資料摘要中。
...
1. 在Adobe Analytics，轉到&#x200B;[!UICONTROL **管理**] > [!UICONTROL **資料源**] > **[!UICONTROL 管理模板]**。

1. 選擇要編輯的模板，然後選擇&#x200B;**[!UICONTROL 編輯]**。

1. 進行任何編輯，然後選擇&#x200B;**[!UICONTROL 保存]**。
