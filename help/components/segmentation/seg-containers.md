...
「點擊」容器定義您要在區段中納入或排除的頁面點擊。「點擊」容器是範圍最小的可用容器，可讓您識別令某條件成立的特定點按和頁面檢視。您可以在此過程中檢視單一追蹤代碼或隔離出網站特定區段內的行為。您可能也會想要在動作發生時精準指出特定值，例如下訂單時的行銷管道。
...
在其他容器中建立區段容器時，本質上是在區段內建立區段。 對巢狀內嵌的容器使用的邏輯如下：
...
- **頁面等於「冬季外套」時的點擊容器**：如果以頁面檢視作為量度，則只會顯示「冬季外套」頁面 (如預期)。如果使用「收入參與率」報告，則沒有頁面會得到評價，因為此維度需要永久性維度。實際購買的頁面檢視（「購買」頁面）未包含在「點選」容器中，因此任何專案都不會獲得收入參與率。 不過，從「造訪」容器執行報告則會納入該次造訪中的所有頁面檢視，並將收入參與值 ($100) 分散給工作階段中檢視的所有頁面。
...
與「頁面」維度相反，「反向連結網域」值會附加至此次造訪中的每個頁面。例如，底下的訪客從參照的網站進入首頁。 因此，在該造訪內的所有頁面都會獲派相同的反向連結網域值。
