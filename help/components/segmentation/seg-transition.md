...
回答有關舊式區段 (2014 年以前建立的舊式區段) 最佳實務的常見問題。

## 管理舊式區段 {#legacy}

+++ **我現有的區段有什麼改變？**

您現有的區段將繼續如往常般運作。任何已套用這些區段的報告都將繼續正常運作。[更多...](/help/components/segmentation/seg-transition.md)

原本預先定義的區段與套裝區段多半會以區段範本的形式移轉至區段產生器。區段範本的用途是快速建立具有常見對象的自訂區段。區段範本無法直接套用至報告，但可以輕易儲存成自訂區段。
...
這些區段會以區段範本的形式移轉至「區段產生器」。已套用這些區段的現有報告將可繼續正常運作。
