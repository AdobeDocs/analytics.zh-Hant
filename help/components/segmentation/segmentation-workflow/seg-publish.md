...
| **[!UICONTROL 建立客群視窗]** | The time frame that you select is used to create the audience on a rolling-calendar basis. For example, **[!UICONTROL Last 30 days]** (default) includes visitors that have qualified for the audience over the last 30 days from today&#39;s date (NOT from the original date when the segment was created). |
...
1. 選取您先前建立要與 [!DNL Audience Manager] 共用之區段的報表套裝資料夾。You see the segment/audience you created. 當您共用區段時，[!DNL Audience Manager] 中會發生下列兩件事：
