...
您可以將Adobe Analytics區段發佈至CX Enterprise。 因此，您可以在Audience Manager和其他啟用管道（包括Advertising、Target和Campaign）中，使用該區段進行行銷活動。

您可以在8小時內將Analytics區段發佈至CX Enterprise。 使用這些區段，在 Audience Manager 中將對象啟動至所有下游目的地。
...
* **成員限制**：透過Adobe Analytics與CX Enterprise共用的對象不能超過2,000萬名獨特成員。
* **資料隱私權**：系統不會根據訪客的驗證狀態篩選客群。 訪客或許可以在未驗證和已驗證狀態下瀏覽您的網站。 訪客未驗證時所發生的動作，仍可能會導致將訪客納入受眾。 請檢閱[Adobe CX Enterprise隱私權](https://www.adobe.com/tw/privacy/experience-cloud.html)，以瞭解對象共用的完整隱私權意涵。
* 如需有關[!DNL Adobe Analytics]與Audience Manager **中區段之間**&#x200B;差異的討論，請參閱[瞭解Analytics與Audience Manager中的區段](/help/integrate/c-audience-analytics/aam-analytics-segments.md)。

## 區段發佈時間軸

| 可用功能 | 可用時間 | 可用位置 |
|---|---|---|
| 中繼資料 (區段標題和定義) | 發佈後立即生效 | Audience Manager， CX Enterprise對象庫， Target |
| 可用的具成員資格區段 | 發佈後 8 小時內 | Audience Manager中的訪客資料檢視器 |
| 特徵與成員母群體 | 24 - 48小時內 | Audience Manager |
...
>[!NOTE]
>
>您&#x200B;**無法**&#x200B;取消發佈下列任何Adobe解決方案目前正在使用的區段： Analytics （在Audience Analytics中）、Campaign、Advertising （適用於核心服務和Audience Manager客戶）以及所有其他外部合作夥伴（適用於Audience Manager客戶）。 您&#x200B;**可以**&#x200B;取消發佈Target正在使用的區段。
...
在Adobe Audience Manager中，當Analytics與CX Enterprise共用區段時，會評估指定區段的訪客清單（含ECID）。
...
1. 選取&#x200B;**[!UICONTROL 對象]**。
1. 在&#x200B;**[!UICONTROL 對象]**&#x200B;頁面上，找出來源為CX Enterprise的對象。 這些對象可在Target活動中使用。
