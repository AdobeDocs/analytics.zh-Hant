...
「頁面逗留時間」[維度](overview.md)會記錄訪客在頁面上逗留的時間量。 此維度使用下列步驟來測算計算：

1. 對於指定的點擊，查看時間戳記。
2. 將此點擊與此次造訪中下一次點擊的時間戳記進行比較。 頁面檢視和連結追蹤點擊都會計入。
...
Adobe會從每次點選與造訪的下一次點選之間經過的時間中，計算此維度伺服器端。 無可設定的變數；可直接用於所有實施作業。
