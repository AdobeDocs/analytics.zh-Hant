...
「造訪深度」 [維度](overview.md)會報告訪客在整個造訪中檢視的頁面檢視次數。 只有在點擊為頁面檢視，且[「頁面」](page.md)維度與最後一個頁面檢視的維度項目不同時，造訪深度才會增加。 這是以造訪為基礎的維度，表示它在整個造訪期間都包含相同的值。 此變數會在造訪結束後，為該造訪中的所有點擊設定。

## 將資料填入此維度中

Adobe會從每次造訪的頁面檢視計算此維度伺服器端。 無可設定的變數；可直接用於所有實施作業。
