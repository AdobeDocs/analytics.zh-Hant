...
此維度會從[內容資料變數](/help/implement/vars/page-vars/contextdata.md) `contextData.['adConsent']`收集資料。 您會將相關的Google同意欄位值填入此變數中： `ad_user_data` （第一個字元）和`ad_personalization` （第二個字元）。 如需詳細資訊，請參閱Google Ads API參考中的[同意](https://developers.google.com/google-ads/api/reference/rpc/v15/Consent)。

| 屬性 | 價值 |
| --- | --- |
| **AppMeasurement變數** | 無（透過`adConsent`內容資料變數設定） |
| **網頁SDK / XDM欄位** | 無 |
| **查詢引數** | 不適用 |
| **XML標籤** | 不適用 |
| **位元組限制** | 100位元組 |
