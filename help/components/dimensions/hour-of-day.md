...
此維度是以報告套裝的時區為依據，而非訪客的當地時區。 例如，若您的報表套裝使用美國山區時間，而加州的訪客在太平洋時間上午10:00造訪您的網站，則點選群組會在維度專案「`11:00 AM`」底下。 如果您想使用會記錄本機訪客時間的維度，Adobe 建議使用 [getTimeParting](/help/implement/vars/plugins/gettimeparting.md) 外掛程式。

## 將資料填入此維度中

此維度衍生自每次點選的時間戳記；沒有可設定的變數。 如上所述，小時反映了報表套裝的時區，而非訪客的當地時區。
