---
title: 造訪次數
description: 了解 Analytics 中的造訪次數量度。 深入了解其計算方式、影響相關行為、如何變更其定義等等。
feature: Metrics
...
* **閒置達 30 分鐘**：幾乎所有的工作階段都會以此方式結束。 若兩次點擊請求之間相隔超過 30 分鐘，就會起始新的造訪。
...
每個相關時段會分別計入一次造訪。 例如，如果訪客在星期一晚上11:45開始導覽您的網站，然後在星期二凌晨12:10傳送其最後一個影像要求，您將會看到歸因於星期一和星期二的造訪。 不過，總造訪次數量度會刪除重複項目，而在專案日期範圍內顯示為單次造訪。

## 維度的造訪次數與總造訪次數

維度內容中的造訪次數 (例如[行銷管道](../dimensions/marketing-channel.md)) 會顯示任何時候包含特定維度項目的造訪。 在同一次造訪中，多個維度項目經常存在於不同的點擊上。 嘗試加總依維度項目報告的造訪次數通常沒有意義。

## 瀏覽 Data Warehouse 中的所有訪客

Data Warehouse提供「造訪 — 所有訪客」量度，不提供「造訪」量度。 「造訪次數 - 所有訪客」量度可與其他 Analytics 工具中的「造訪次數」量度比較。 Data Warehouse 中的「造訪次數」量度會排除沒有永久性 Cookie 的訪客。 在需要以造訪次數作為量度的 Data Warehouse 請求中，Adobe 建議使用「造訪次數 - 所有訪客」的量度。
