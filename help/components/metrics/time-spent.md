...
| [!UICONTROL 平均頁面逗留時間] | 已棄用的量度。<br> 反之，如果需要維度專案的平均時間，Adobe建議您使用[[!UICONTROL 網站平均逗留時間]](average-time-on-site.md)。 | Report Builder (請求中包含維度時) |
...
| prop1 | 總逗留秒數 | 每次造訪逗留時間 | 每位訪客逗留時間 | 序列計數 | 網站平均逗留時間 |
|---|---|---|---|---|---|
...
| eVar1 | 總逗留秒數 | 每次造訪逗留時間 | 每位訪客逗留時間 | 序列計數 | 網站平均逗留時間 |
|---|---|---|---|---|---|
...
* 花費秒數是指目前點選時間戳記與下次點選時間戳記之間的差異。 因此，造訪的最後一次點選（和跳出）沒有逗留時間。
