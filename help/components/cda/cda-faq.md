...
在某些情況下，個別使用者可與大量 ECID 建立關聯。如果個人使用許多瀏覽器或應用程式，就可能發生這種情況，如果他們經常清除 Cookie 或使用瀏覽器的私人或無痕瀏覽模式，更會加劇此情形。裝置圖表將綁定一個特定使用者 ID 的 ECID 數量上限設為 200。如果使用者 ID 與太多 ECID 建立關聯，設備圖表就會假設該使用者 ID 無效，並刪除與該使用者 ID 相關聯的叢集。然後，將用戶ID添加到塊清單中，以防止其在將來被回收。 CDA 中的結果為使用者 ID 的行為不會跨裝置連結。
