...
跨裝置分析 (CDA) 功能將 Analytics 從裝置導向檢視轉變為以人為導向的檢視。因此分析師可以瞭解跨瀏覽器、裝置或應用程式的使用者行為。Adobe支援[欄位式銜接](field-based-stitching.md)，此銜接僅使用確定性比對將裝置連結在一起。
...
使用跨裝置分析需要[欄位式拼接](field-based-stitching.md)。
...
跨裝置分析是一項具突破性且完善的功能，但其使用方式有所限制。[依欄位彙整](field-based-stitching.md)也有自己的特定限制。
