...
* **如果使用依欄位拼接**，您選擇用來協助識別登入使用者的 prop 或 eVar 會覆寫其他識別碼。共用裝置會視為是個別人員，即使來自相同裝置亦然。
...
針對依欄位匯整，識別碼自訂變數會區分大小寫。 如果識別碼值不符合大小寫，「已識別人員」量度的數量可能會高得多。 例如，若 `bob` 和 `Bob` 會傳送，且預期會是相同的人，CDA會將這兩個值解譯為不同的值。

## 檢視識別碼prop/eVar時，為何會看到「未識別人員」量度的非零值？

當訪客在報表視窗中產生已驗證和未驗證的點擊時，通常會發生此情況。 此訪客同時屬於 [已識別狀態](/help/components/dimensions/identified-state.md) 維度，導致識別碼中未識別點擊的歸因。 此情境可在 [重播](replay.md) 執行，視重播頻率和成功率而定。