...
您可將[!UICONTROL Flow]視覺化與「行動裝置類型」維度搭配使用。
...
原使用自訂訪客 ID 的客戶可升級至 CDA，不須變更實施作業。`visitorID` 變數仍用於來源報告套裝中。但如果使用者驗證，CDA 會忽略虛擬報告套裝中的 `visitorID` 變數。
...
客戶服務可要求從裝置圖表切換為現場拼接，反之亦然。 但是，要完成這種交換機可能需要數週或更長的時間，並且來自前一種方法的&#x200B;*歷史銜接資料將丟失。*
...
## 我公司的套報告套裝有多少套可以啟用 CDA？

可以啟用多個報告套裝，但若一次請求多個報告套裝，則每個額外的報告套裝都會增加整體佈建時間。 CDA 不會合併多套報告套裝。每個啟用 CDA 的報告套裝都必須具備跨裝置性質 (包含來自多個表面的資料，例如桌面網頁、行動網頁、行動應用程式等)。

## 如果我的 Experience Cloud 組織 (亦即 IMS 組織) 在不同地區擁有許多家公司，我是否可為所有公司啟用 CDA？

不可以，若是同一個組織，只有一個地區可以啟用 CDA。

## 與1天重播相比，7天重播有哪些優點和缺點？
