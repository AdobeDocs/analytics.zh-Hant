...
* 使用排除處理。 如果「電子郵件」和「附屬機構」管道使用相同的查詢字串參數，但您只有很少的電子郵件追蹤代碼，則可以在定義電子郵件的規則集中指定電子郵件追蹤代碼。 然後分類後者所有其他追蹤碼：*`affiliates.`*
* 在您的電子郵件系統中，新增查詢字串參數至所有著陸頁面 URL，例如 *`&ch=eml`*。 建立一個規則集，用於偵測 ch 查詢參數是否等於 *`eml`*。 如果不包含 *`eml`*，則為附屬機構。

## 反向連結網域包含的資料多於我的預期。

處理規則清單中的反向連結網域可能太高。 這應該是最後（或最後一個）規則集之一，因為處理順序很重要。

## 我已經建立了符合查詢字串參數的規則，但不管用。

請務必在查詢字串引數欄位中指定引數名稱（通常是英數字元）。 此外，請確定引數值是在運運算元之後指定，如下列電子郵件規則範例所示。

![](assets/example_email.png)

## 為何我所有的上次接觸流量均歸屬於內部網域？

您有一個符合內部流量的規則。 請注意，該等規則會處理訪客在您網站上的每一次點按，而不僅是首次造訪。 如果您有如「*`Page URL exists`*」的規則，而沒有其他標準，則該管道會符合網站上的每個後續點按，因為頁面 URL 永遠存在。

## 我如何對報告的「未識別管道」中顯示流量進行除錯？

規則會依序處理。 如果不符合特定標準，點按便會屬於下列三個類別之一：
...
當規則不擷取資料時，或如果規則設定有誤，則報告會顯示報告上[!UICONTROL 「未識別管道」]行中的資料。 例如，在處理順序結束時，您可以建立名為&#x200B;*Other*&#x200B;的規則集，該規則集也會識別內部流量。
