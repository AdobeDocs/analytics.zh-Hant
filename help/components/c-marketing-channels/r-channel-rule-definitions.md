...
seo-title: 行銷管道處理規則-定義
solution: Analytics
subtopic: 行銷渠道
title: 行銷管道處理規則-定義
topic: Reports & Analytics
uuid: 4e71ff5b-912a-4dc0-4c22-4be74c5cc0
...
<table id="table_C18A0F1C9E214EB585A29801BA2400F8"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Advertising Cloud和Advertising Analytics整合所使用的主要追蹤代碼。啓用其中一項整合後，就可使用追蹤代碼首碼來識別Advertising Cloud專用頻道。使用「AMO ID」開頭為「AL」，「AC」為「AC」，Social則為「AO」。在行銷渠道中使用AMO ID時，點按/成本/曝光度量可歸因於正確的頻道(未設定時，這些量度將前往「直接」或「無」)。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>AMO ED ID </p> </td> 
   <td colname="col2"> <p>Advertising Cloud使用的次要追蹤代碼。此追蹤代碼的主要目的在於作為將資料傳送回Ad Cloud的索引鍵。但如果您想將這些檢視視為兩個單獨的行銷渠道，也可以用來識別顯示點進次數和顯示檢視次數。This can be done by set the marketing channel logical for「AMO EF ID」end was」：d「顯示點進次數」或「AMO EF ID」結尾為結尾：i「用於顯示檢視次數」。如果您不想分割顯示為兩個渠道，請改用AMO ID維度。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>具備多個可用選擇，包括: </p> <p> 
     <ul id="ul_FE39B5F36235441FB757CC73CA2C4F51"> 
      ...
      <li id="li_25EDA39748D141BA8173CC4C41035ABA"> <p> <span class="uicontrol"> 不包含 </span>：例如，讓您指定反向連結網域不包含特定值(相對於使用選取 <span class="term"> 範圍包含 </span>)。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>反向連結 URL 與一個內部 URL 篩選器不相符，並且這是為「管理工具」的報表套裝而定義的。此設定可搭配使用<span class="term"> 頁面URL </span> 和 <span class="term"> 存在 </span> 以設定所有的規則，因此不會在報表的 <a href="../../components/c-marketing-channels/c-faq.md#section_451E42994DA247A8A7B8559C715A5EE7" type="section" format="dita" scope="local"> 「未識別渠道」 </a> 區段中著陸。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>使用 Adobe 網站信標標記的網頁名稱。此值等於  <span class="varname"> s.pageName </span>. Examples include <span class="varname"> Home Page </span> and <span class="varname"> About Us </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>搜尋關鍵字和搜尋引擎的串連，以唯一識別搜尋引擎。例如，如果搜尋 computer，則搜尋引擎和關鍵字的識別如下所示: </p> 
    <code>搜尋追蹤代碼=「&lt; search_ type&gt;：&lt;搜尋引擎&gt;：&lt; search keyword&gt;」search_ type=「n」或「p」，search_ engine=「Google」，and search_ keyword=「computer」 </code>
  <p><b>注意：</b> n=自然；p=付費 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>設定該渠道的值為 </p> </td> 
   <td colname="col2"> <p>除了瞭解為您網站帶來訪客的行銷渠道之外，您還能知道渠道內的哪些橫幅廣告、搜尋關鍵字或電子郵件促銷活動為訪客的網站活動獲得評價。該 ID 是與渠道一同儲存的渠道值。該值通常為內嵌於著陸頁面或反向連結 URL 中的促銷活動 ID，有時也是搜尋引擎和搜尋關鍵字組合，或是最近識別特定渠道中訪客的反向連結 URL。 </p> </td> 
  </tr> 
 </tbody> 
</table>
