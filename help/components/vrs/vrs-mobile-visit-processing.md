...
由於背景點擊處理僅可使用報表時間處理提供給虛擬報表套裝使用，因此 Adobe Analytics 支援兩種處理背景點擊的方式，以保留基本報表套裝 (不使用報表時間處理) 中的造訪次數。若要存取此設定，請導覽至「Adobe Analytics管理工具」，前往適用基本報表套裝的設定，然後導覽至「行動管理」功能表，然後導覽至「行動應用報表」子功能表。
