...
![報告時間處理管道](assets/google2.jpg)

此處理架構可提供更具彈性的報告選項。例如，您可以以非破壞性方式將訪問超時時間段更改為所需的任意時間段，這些更改將反映在整個報告期的eVar持久性和段容器中。 此外，您可以建立任意數量的虛擬報表套裝，並根據相同的基本報表套裝，讓每個套裝使用不同的「報表時間處理功能」選項，而不變更基本報表套裝中的任何資料。

[!UICONTROL 報告時間處理] 還允許Analytics阻止啟動新訪問時的背景點擊，並允許 [Adobe Experience Platform移動SDK](https://experienceleague.adobe.com/docs/mobile.html?lang=zh-Hant) 啟動應用程式啟動事件時啟動新訪問。
...
* **具有 (低流量) 或「已超出不重複限制」的維度：**(低流量) 條列項目在使用報告時間處理時的決定稍有不同，並且不能保證符合在基本報表套裝上產生報表時所觀察到的結果。不屬於「低流量」的Dimension行項不保證表示該行項的資料的100%。 這些差異可能會隨著維中唯一值的數量的增加而變得更加明顯。
