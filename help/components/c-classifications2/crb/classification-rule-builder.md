...
* 分類規則產生器 (CRB) 要求匯出時，會同時提取分類和未分類的值，未分類的值會在匯出結束前傳遞。這表示隨著時間推移，最多可填入 1000 萬個分類值，永遠不需要尋找未分類的值。
...
| 步驟 | 執行位置 | 說明 |
|--- |--- |--- |
| 步驟 1 (必要條件): [設定分類結構](https://marketing.adobe.com/resources/help/en_US/reference/c_classifications.html)。 | [!UICONTROL 管理員] &gt; [!UICONTROL 報表套裝] &gt; [!UICONTROL 編輯設定] &gt; &lt;流量分類或轉換分類&gt; | 選擇變數並定義用於該變數的分類。<br>變數至少必須先建立一個分類欄，才能夠在規則中使用。<br>啟用分類後，您可使用匯入工具和規則產生器來分類特定值。 |
