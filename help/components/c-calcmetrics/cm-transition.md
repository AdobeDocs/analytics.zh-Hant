...
seo-title: 常問的問題
title: 常問的問題
uuid: 9b7f1cd1-b969-4b15-8ab1-969d816 b65 b
...
[為什麼會看到對其他報表套裝無效的「未知」計算量度 (即使可對這些報表套裝建立和套用這些量度)?](../../components/c-calcmetrics/cm-transition.md#section_6772818EFDED46E9B7095D64C3B77211)

[為什麼我對舊式計算量度進行的變更無法儲存?](../../components/c-calcmetrics/cm-transition.md#section_81CDEFCA1FD542579AF183DA1494EAF0)

[為什麼我的計算量度不會顯示在行銷管道報表中?](../../components/c-calcmetrics/cm-transition.md#section_FC350359A775433AB5F43C7CAB304D62)
...
> [!NOTE] 即使計算量度不再系結至特定報表套裝，而且可用於登入公司可見的任何報表套裝，計算量度建立或上次儲存的報表套裝仍會顯示在計算量度管理員中。

> [!NOTE] 即使刪除計算量度，參考該量度的書簽或控制面板報表仍會運作。
...
>[!NOTE]
>
>如果計算量度用於書簽、控制面板、警報或排程報表中，編輯新的副本將不會影響舊持續計算量度。
...
現在ReportSuite CalculatedMetrics API方法(1.3或1.4)將繼續儲存、刪除和擷取使用舊商店的計算量度。現有的計算量度則會移轉並顯示在新的計算量度產生器中。**使用 API 方法建立的新計算量度只會顯示在 API 中。仍可在報告 API 中使用。**
...
## 為什麼我的計算量度不會顯示在行銷管道報表中? {#section_FC350359A775433AB5F43C7CAB304D62}
...
您需要明確儲存區段。包含內嵌區段定義的現有計算量度仍可在 Ad Hoc Analysis 中正確執行並加以檢視，但若未明確儲存區段，將無法儲存該量度。

## (僅適用於 Report Builder) 為什麼計算量度沒有出現在我的請求中? {#section_DA4792FE5D7945218CD5E6328DE08E82}

如果請求是在v5.2中建立且包含計算量度，則這些量度不會顯示在v5.1(或更早版本)中。這是因為計算量度現在使用全域 ID (非報表套裝專屬 ID)。
...
[!UICONTROL 「報告與分析」] 在 [!UICONTROL 「報告與分析]」中顯示計算量度總計時，只是將公式套用到總計而已。例如，計算量度「訂購/造訪」的總計是將「訂購總量」除以「存取總數」。但是在某些情況下，計算量度總計不只是行項目的總和，而是網站的總計。
