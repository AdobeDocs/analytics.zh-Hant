---
description: （選用）將分類匯入行銷報表之前，您可以下載範本來協助您建立分類資料檔案。 該資料檔案會使用您所需的分類當做欄標題，然後將報告資料集組織在適當的分類標題下。
title: 分類範本
...
（選用）將分類匯入行銷報表之前，您可以下載範本來協助您建立分類資料檔案。 該資料檔案會使用您所需的分類當做欄標題，然後將報告資料集組織在適當的分類標題下。

**[!UICONTROL 管理員]** > **[!UICONTROL 分類匯入工具]**。

| 元素 | 說明 |
| --- | ---|
| 選取報表套裝 | 在範本中選取要使用的報告套裝。 報告套裝和資料集必須相符。 |
| 要分類的資料集 | 選取資料檔案的資料型別。 功能表含有報表套裝中所有已針對分類設定的報告。 |
| 匯出數值 2 | **重要事項**：此選項不適用於已啟用「新分類架構」的報表套裝。 |
| 編碼 | 選取資料檔案的字元編碼。 預設編碼格式為UTF-8。<br>**重要**：此選項不適用於已啟用「新分類架構」的報表套裝。 |
| 下載 | 下載範本檔案。 |

範本包含特定資料集的目前定義的分類（欄標題），但不包含與每個分類相關聯的資料。
