...
單一維度可以具有多個分類維度。 例如，您可以使用其他產品屬性來分類[!UICONTROL 產品 ID]，例如產品名稱、顏色、尺寸、說明以及 SKU。 上述每一個屬性都會是獨立的分類維度，且隸屬於同一個上層維度。 使用這些屬性來強化您的報告，可提供更深入且更複雜的報告機會。 一旦維度包含分類資料，分類維度就會在僅包含分類維度項目的報告中提供使用。 任何未包含分類值的上層維度項目，皆會歸類為[未指定](/help/technotes/unspecified.md)。
