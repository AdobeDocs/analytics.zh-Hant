...
分類集會提供管理分類和規則的單一介面。此工作流程將在[»報告套裝設定](/help/admin/tools/manage-rs/report-suites-admin.md)中建立分類的功能與[分類匯入工具](/help/components/classifications/sets/manage/set-manager.md)結合。結果是提供一個直覺易用的介面，可用來建立和管理分類資料。
...
* [**[!UICONTROL 分類集]**](manage/set-manager.md)：建立、編輯和刪除分類集。
...
1. [建立分類集](/help/components/classifications/sets/manage/create.md)，使用一個或多個報告套裝的訂閱，以及用來識別產品的主要維度組合。例如：
