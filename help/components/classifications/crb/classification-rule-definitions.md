...
<table id="table_2B3A8BB7BDE14836ACA6A1D444B011CD"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> （預設設定）一律覆寫現有的分類索引鍵，包括透過匯入工具(SAINT)上傳的分類。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>規則僅會覆寫未設定的值 </p> </td> 
   <td colname="col2"> <p>僅填入空白（未設定）的儲存格。 現有分類不會變更。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>回顧時間範圍 </p> </td> 
  ...
  <tr> 
   ...
   <td colname="col2"> 可讓您指定規則處於草稿模式。 草稿狀態可讓您在執行規則之前先測試規則。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>指定發生符合條件時所要採取的動作。 </p> <p>例如，您將促銷活動名稱設為$2，這會識別追蹤代碼中的位置2為促銷活動名稱。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>每個規則集皆會套用至特定變數。 有效的選項有： </p> 
  ...
  <tr> 
   ...
   <td colname="col2"> 您在索引鍵中尋找的文字模式。 這些條件可以是搜尋詞、字元或規則運算式。 </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> 如果符合匹配准則，您要為選取的分類欄指定的值。 </td> 
  </tr> 
  <tr> 
...
| 元素 | 說明 |
|---|---|
| 範例密鑰 | 要使用的測試字串。 例如，您可以從追蹤程式碼中的特定字元建立分類。 您可以比對特定字元、單字或字元模式。 |
| 匹配群組 | 顯示規則運算式對應到促銷活動ID字元的情形，以便您能夠將促銷活動ID中的位置分類。 |
| 匹配結果 | 顯示成功符合規則運算式的字串部分。 |
...
| 元素 | 說明 |
|---|---|
| 執行測試 | 測試規則集時，請使用報表中的索引鍵來檢視規則集將如何影響索引鍵。 |
| 篩選 | 篩選[!UICONTROL 結果]面板中的值。 |
