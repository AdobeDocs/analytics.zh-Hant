...
若要設定新的雲端匯入或匯出位置，或編輯現有的雲端匯入或匯出位置：
...
1. 在「[!UICONTROL **位置屬性**]」部分，指明您位置帳戶特定的帳戶類型資訊。
   ...
   如需設定指示，請展開以下與您在所選帳戶型別對應的區段 [!UICONTROL **位置帳戶**] 欄位。 （也可使用其他舊帳戶型別，但不建議使用。）
   ...
   **帳戶型別**
   ...
   | 欄位 | 函數 |
   |---------|----------|
   | [!UICONTROL **貯體名稱**] | 在您的 Amazon S3 帳戶內，您希望 Adob&#x200B;&#x200B;e Analytics 資料傳送的貯體。 |
...
1. 選取「[!UICONTROL **儲存**]」。
   ...
   您現在可以將資料匯入或匯出至您設定的帳戶和位置，或從這些帳戶和位置匯出資料。
