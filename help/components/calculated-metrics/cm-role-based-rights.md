...
| 非管理員層級使用者 | 依預設，使用者可以建立計算量度。 但是，這些許可權可能會受到管理員的限制。 | 僅可與個別使用者共用 | 僅可檢視/編輯/刪除/等等自己的計算量度。 非管理員使用者必須擁有所有元件事件的存取權，才能檢視共用量度（Admin Console中的許可權仍強制執行）。  如果與非管理員使用者共用儀表板或排程報告，但未與他們共用量度，則報表會在套用量度的情形下執行 (假設該使用者具有檢視事件的權限)。 但是，他們將無法檢視定義或編輯量度。 | 只能使用已核准的計算量度，無法標籤為已核准。 | 可以套用他們自己的計算量度以及與他們共用的區段。 |
