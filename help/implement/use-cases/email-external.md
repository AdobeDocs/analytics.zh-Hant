...
公司可使用 Analytics 來判斷電子郵件行銷活動是否成功。

[!DNL Analytics] 可用數種關鍵度量來報告電子郵件行銷活動分析資料，其中包括：
...
要順利顯示電子郵件行銷活動分析資料，必須執行數個步驟。這些步驟說明如下：

1. 建立唯一的追蹤代碼。

   使用者常會要求我們提供關於每個特定行銷活動的追蹤建議。這完全取決於使用者，端視何者效果最佳而定。每個使用者都是不同的。Adobe 建議每位使用者建立好記的追蹤代碼，如下列範例所示：

   * sc_cid=A1123A321 > &quot;A&quot; 會標記附屬機構行銷活動
   * sc_cid=EM033007 > &quot;EM&quot; 會標記電子郵件行銷活動
   * sc_cid=GG987123 > &quot;GG&quot; 表示 Google，屬於付費搜尋行銷活動
...
1. 更新 JavaScript 程式庫。
   ...
   1. 確定「貼上程式碼」JavaScript 標記位於所有的登陸頁面上。此「貼上程式碼」必須參考在「A 部分」中更改的 [!DNL s_code.js] 版本。
      ...
      * [!UICONTROL sc_cid] 參數是查詢字串參數的範例之一。任何查詢字串參數皆可由 [!UICONTROL getQueryParam] 外掛程式使用及擷取。請確定查詢字串參數僅用於行銷活動追蹤。只要這些參數出現在查詢字串中，其值就會複製到     *`campaign`*。

1. 使用 [!UICONTROL SAINT] 為行銷活動追蹤代碼進行分類。

   [!UICONTROL 「SAINT 行銷活動管理工具」]可用來將追蹤代碼轉換為好記的名稱。它也可用來摘要每個電子郵件行銷活動的成就。下方的步驟 5 中列出了設定電子郵件行銷活動時所須執行的程序。

1. 參閱電子郵件行銷活動的路徑分析 (選用)。

   電子郵件行銷活動的路徑分析在執行時可比照其他行銷活動的路徑分析。您可以使用變數來顯示行銷活動的路徑分析，如下列步驟所說明：

   1. 請向 Adobe [!DNL Customer Care] 洽詢為[!UICONTROL 「自訂分析」]變數 (prop) 開啟路徑分析的相關事宜。

   1. 在所有頁面上，將頁面名稱複製到指定的 [!DNL s.prop] 中。
   1. 在電子郵件登陸頁面上，將電子郵件行銷活動的名稱附加到 prop。結果顯示如下：
