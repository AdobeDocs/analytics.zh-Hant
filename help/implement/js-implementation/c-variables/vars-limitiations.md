...
keywords: Analytics實作；變數；限制；limits
...
| cookieDomainPeriods | 判斷頁面 URL 的網域中所含的時段數，以決定 [!DNL Analytics][!UICONTROL  訪客 ID] (s_vi) Cookie 設定所在的網域。For `www.mysite.com`, *`cookieDomainPeriods`* should be "2." For `www.mysite.co.jp`, *`cookieDomainPeriods`* should be "3." |
| cookieLifetime | 讓 JavaScript 和 [!DNL Analytics] 伺服器用來決定 Cookie 的有效期限。 |
| cookiesEnabled | 指出第一方工作階段 Cookie 是否可由 JavaScript 設定。 |
| currencyCode | 決定收入在進入 [!DNL Analytics] 資料庫時所要套用的轉換率。[!DNL Analytics] 資料庫會以您所選擇的貨幣來儲存所有金額。If that currency is the same as that specified in *`currencyCode`*, or *`currencyCode`* is empty, no conversion is applied. |
| dc | 可讓您設定您的資料所要傳送到的資料中心。 |
| doPlugins | *`doPlugins`* 是 *`s_doPlugins`* 函數的參考。It allows the *`s_doPlugins`* function to be called at the appropriate location within the JavaScript file. |
| dynamicAccountList | 動態選取資料所要傳送到的報表套裝。此&#x200B;*`dynamicAccountList`* 變數包含用來決定目標報表套裝的規則。 |
| dynamicAccountMatch | 使用 DOM 物件來擷取套用了 *`dynamicAccountList`* 中所有規則的 URL 區段。This variable is only valid when *`dynamicAccountSelection`* is set to 'True.' |
...
| 首頁 | (在 Internet Explorer 中) 指出目前的頁面是否設為使用者的首頁。 |
...
| linkName | 一個用於[!UICONTROL 連結追蹤]中的可選變數，可判斷自訂、下載或退出連結的名稱。此&#x200B;*`linkName`* 變數通常不需要，因為 *`tl()`* 函數中的第三個參數會取代它。 |
| linkTrackEvents | 包含應隨自訂、下載和退出連結而傳送的事件。只有在 *`linkTrackVars`* 包含「事件」時，才會考量此變數。 |
| linkTrackVars | 以逗號分隔，會隨自訂、退出與下載連結而傳送的變數清單。若&#x200B;*`linkTrackVars`* 設為 ""，則所有擁有值的變數都會隨連結資料傳送。 |
| linkType | 一個用於連結追蹤中的可選變數，可判斷會出現連結名稱或 URL 的報表 (自訂、下載或退出連結)。*`linkType`* 通常不需要，因為 *`tl()`* 函數中的第二個參數會取代它。 |
...
| 行動 | 控制使用 Cookie 和訂閱者 ID 來識別訪客的順序。 |
...
| pageURL | 極少數的情況下，頁面的 URL 不會是您要在 [!DNL Analytics] 中報告的 URL。To accommodate these situations, [!DNL Analytics] offers the *`pageURL`* variable, which overrides the actual URL of the page. |
| 外掛程式 | 在 Netscape 和 Mozilla 瀏覽器上，列出瀏覽器中所安裝的外掛程式。 |
| 產品 | 用來追蹤產品和產品類別，以及購買數量和購買價格。此&#x200B;*`products`*&#x200B;變數一律應與成功事件一起設定。Optionally, the *`products`* variable can track custom numeric and currency events, as well as [!UICONTROL Merchandising] eVars. |
| propN | 用來建置 [!DNL Analytics][!UICONTROL  流量模組]內的自訂報表。[!UICONTROL prop] 可作為計數器 (用以計算頁面檢視的傳送次數)、用於路徑分析報表，或用於關聯報表中。 |
| purchaseID | 讓 [!DNL Analytics] 用來防止重複計算同一份訂單。Whenever the purchase event is used on your site, you should use the *`purchaseID`* variable. |
| 反向連結 | 還原遺失的反向連結資訊。 |
| 解析度 | 顯示檢視網頁之訪客的螢幕解析度。 |
| 伺服器 | 顯示網頁的網域 (以指出訪客進入的網域) 或為頁面提供服務的伺服器 (以供負載平衡快速參考之用)。 |
| state | 擷取網站訪客居住所在的州別。 |
| trackDownloadLinks | 若您要在網站上追蹤可下載檔案的連結，請將 *`trackDownloadLinks`* 設為 'true'。If *`trackDownloadLinks`* is 'true,' *`linkDownloadFileTypes`* determines which links are downloadable files. |
...
| transactionID | 將離線資料連結至線上交易 (例如在線上產生的潛在客戶或購買)。每個傳送到 Adobe 的唯一&#x200B;*`transactionID`* 都會被記錄下來，以備[!UICONTROL 「資料來源」]上傳有關該次交易的離線資訊時使用。請參閱[資料來源指南](https://marketing.adobe.com/resources/help/en_US/sc/datasources/)。 |
| s_usePlugins | 若 *`s_doPlugins`* 函數可供使用並包含有用的程式碼， [!UICONTROL s_ usePlugins] 應設為'true'。[!UICONTROL 當usePlugins] 為'true'時， *`s_doPlugins`* 會在每個影像要求之前呼叫函數。 |
...
| visitorNamespace | If *`visitorNamespace`* is used in your JavaScript file, do not delete or alter it. 此變數可用來識別 Cookie 設定所在的網域。若&#x200B;*`visitorNamespace`* 有所變更，則 [!DNL Analytics] 中報告的所有訪客都可能變成新訪客。簡言之，未經 Adobe 顧問許可，請勿更改此變數。 |
| zip | 擷取網站訪客居住地的郵遞區號。 |
