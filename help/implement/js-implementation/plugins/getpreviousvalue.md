...
topic: 開發人員和實施
uuid: 20da7b4a-9820-4690-a1 cc-d10 b6 d627 a7
...
1. The events that must be set on the page view in order to trigger the retrieval of the previous value ( *`event1`* above). 若保留為空白或省略，外掛程式則會擷取所有頁面檢視上的先前值。
...
* 由於在執行程式碼的外掛程式區段時必須呼叫函數，因此每當呼叫&#x200B;*`s.t()`* 或被 *`s.tl()`* 呼叫。

* 選擇的變數應在呼叫 *`s.getPreviousValue`*. Because the *`s_doPlugins()`* function is executed after the variables on the page are populated, this issue rarely occurs. It should only be a matter of concern if the variable used with this plug-in is populated within the *`s_doPlugins()`* function and after the call to *`s.getPreviousValue`*.
