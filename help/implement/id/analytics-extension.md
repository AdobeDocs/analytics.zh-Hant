...
Adobe Analytics標籤擴充功能可讓您使用標籤管理介面實施AppMeasurement。 由於此標籤擴充功能本質上是隱藏的AppMeasurement，因此提供類似方法來識別訪客，並需要個別標籤擴充功能來收集ECID。

## 使用訪客ID服務識別訪客（建議）

若要使用Adobe Analytics標籤擴充功能來使用訪客ID服務，請在您的標籤屬性中加入[!UICONTROL Experience Cloud ID服務]標籤擴充功能（其實作訪客ID服務）。
