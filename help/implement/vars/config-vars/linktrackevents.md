...
## 連結追蹤呼叫中使用Adobe Experience Platform中標籤的事件

如果您不使用自訂程式碼，Adobe Experience Platform會自動在連結追蹤點擊中加入已定義的事件。

>[!IMPORTANT]
>
>如果您使用自訂程式碼編輯器在資料收集UI中設定事件，也必須使用自訂程式碼在`linkTrackEvents`中納入事件。

## AppMeasurement 和 自訂程式碼編輯器中的 s.linkTrackEvents
...
此變數的預設值為空字串。如果此變數未定義，所有事件都會納入連結追蹤影像要求中。請注意，資料收集會根據介面中設定的事件自動填入此變數，因此一律會針對在Adobe Experience Platform中使用標籤的實作設定。
