...
AppMeasurement 提供自動追蹤連結的功能，讓您追蹤指向網站外部的連結。如果 [`trackExternalLinks`](trackexternallinks.md) (AppMeasurement)或 [`clickCollectionEnabled`](trackexternallinks.md) 啟用Web SDK)後，當訪問者按一下連結離開您的站點時，映像請求將發送到Adobe。 `linkExternalFilters` 和 [`linkInternalFilters`](linkinternalfilters.md) 變數決定要將哪些連結視為內部/外部連結。

如果此變數包含值，則自動退出連結跟蹤的行為與允許清單類似。 如果連結點擊與任何 `linkExternalFilters` 值不符，則不視為退出連結。系統會針對此變數檢查整個 URL。如果啟用 [`linkLeaveQueryString`](linkleavequerystring.md)，也會檢查查詢字串。
...
## AppMeasurement中的s.linkExternalFilters和分析擴展自定義代碼編輯器
