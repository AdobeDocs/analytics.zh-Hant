...
如果此欄位留空，其預設值為 `[rsid]sc.adobedc.net`。
...
>[!TIP]
>
> 有些實作會將資料指向 `2o7.net`。雖然這是有效的資料彙集網域，不過此網域不會使用區域資料彙集。使用 `2o7.net` 的實施會出現略高的影像要求回應時間。
...
如果您使用第三方 Cookie 實施，`trackingServer` 的值為 `data.adobedc.net` 的子網域。例如：
...
您的組織可能已在或網域中使用第三方追蹤 `sc.adobedc.net` 伺 `2o7.net` 服器。  這些主要用於舊版Adobe Analytics，而且仍然有效。
