...
AppMeasurement 提供自動追蹤連結的功能，讓您追蹤指向網站外部的連結。若 [`trackExternalLinks`](trackexternallinks.md) (AppMeasurement)或 [`clickCollectionEnabled`](trackdownloadlinks.md) (Web SDK)啟用後，當訪客按一下連結離開您的網站時，影像要求會立即傳送至Adobe。 [`linkExternalFilters`](linkexternalfilters.md) 和 `linkInternalFilters` 變數決定要將哪些連結視為內部/外部連結。

如果此變數包含值，自動退出連結追蹤會表現出類似區塊清單的行為。 如果連結點擊與任何 `linkInternalFilters` 值不符，則會視為退出連結。系統會針對此變數檢查整個 URL。如果啟用 [`linkLeaveQueryString`](linkleavequerystring.md)，也會檢查查詢字串。
...
如果連結目標網域與目前的網域不同，連結會自動符合退出連結的資格 `window.location.hostname`. Web SDK不提供任何設定變數來修改自動退出連結偵測。 如果您需要自訂符合退出連結資格的網域，可在 `onBeforeEventSend` 回呼。

請參閱 [自動連結追蹤](https://experienceleague.adobe.com/docs/experience-platform/edge/data-collection/track-links.html#automaticLinkTracking) 如需詳細資訊，請參閱網頁SDK檔案。

## 對外連結 — 永不使用Adobe Analytics擴充功能追蹤
...
## AppMeasurement和Analytics擴充功能自訂程式碼編輯器中的s.linkInternalFilters
