...
> [!NOTE] 如果此變數在點擊中設定，而變數中沒有購物車事 [`events`](events/events-overview.md) 件，則「產 [品檢視](/help/components/metrics/product-views.md) 」量度會遞增1。 請確定您使用變數在每次點擊上設定了適當的購物車 `products` 事件。
