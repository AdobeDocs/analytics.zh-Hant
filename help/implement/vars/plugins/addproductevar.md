...
外 `addProductEvar` 掛程式可讓您輕鬆將使用產品語法的Adobe Analytics銷售eVar新增至產品變數，而不需擔心產品變數的現有內容是否會變更／移動／刪除。 如果您想要輕鬆將產品語法銷售eVar新增至變數，Adobe建議使用此外掛程 `products` 式。 如果您不搭配產品語 `addProductEvar` 法使用銷售eVar，就不需要使用外掛程式。

> [!NOTE] 此外掛程式不會取代產品項目中已存在的eVar。 它只會附加您使用此外掛程式設定的值。 在附加該產品已存在的eVar時請務必小心。
...
1. 如果您尚未建立新的啟動規則，請使用下列設定建立名為「初始化外掛程式」的啟動規則：
   * 條件：無
   * 事件：核心——載入的程式庫（頁面頂端）
1. 使用下列設定將動作新增至上述規則：
...
外掛 `addProductEvar` 程式使用下列引數：

* **`en`**（必要，字串）:要新增至目前包含在產品變數中的最後一個項目的eVar。 如果產品變數空白，外掛程式會建立「空白」產品項目，其eVar值會附加至項目結尾。
* **`ev`**（必要，字串）:指派給eVar的值。
* **`ap`**（可選，布林）:如果產品變數目前包含多個產品項目，則值true（或1）會將eVar新增至所有**&#x200B;產品&#x200B;**項目。  預設為false（或0），這會將eVar新增至僅包含在產品變**&#x200B;數中的&#x200B;**最後一個項目。

外掛 `addProductEvar` 程式不會傳回任何內容。 而是將和引數中指定的eVar（和eVar值） `en` 新 `ev` 增至變 `products` 數。
