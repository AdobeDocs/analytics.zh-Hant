...
外 `addProductEvar` 掛程式可讓您輕鬆將使用產品語法的Adobe Analytics銷售eVar新增至產品變數，而不需擔心產品變數的現有內容是否會變更／移動／刪除。 如果您想要輕鬆將產品語法銷售eVar新增至變數，Adobe建議使用此外掛程 [`products`](../page-vars/products.md) 式。 如果您不搭配產品語 `addProductEvar` 法使用銷售eVar，就不需要使用外掛程式。
