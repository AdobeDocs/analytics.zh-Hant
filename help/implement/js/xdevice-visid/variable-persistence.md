...
* 如果描述檔存在，系統中已有的訪客描述檔會從此開始使用，而不再使用先前的訪客描述檔。
* 如果找不到相符的訪客ID，則會建立新的訪客資料。

當未驗證的客戶首次造訪您的網站時，Adobe Analytics會為該客戶指派訪客個人檔案。 當建立新的訪客資料時，某個瀏覽會結束，而另一個瀏覽則會開始。
...
在包含先前所無法識別之 `s.visitorID` 值 (`u999` 以上) 的第一個資料連線上，建立一個新的訪客資料。先前描述檔的永久值會傳送至新描述檔。

* 設為瀏覽時即過期的 eVar 不會複製至已驗證的訪客資料。請注意，值 `car` 以上不會持續存在。
* 設為透過其他措施來過期的 eVar 將複製至已驗證的訪客資料。請注意，值 `apple` 持續存在。
* 對於持續存在的 eVar，不會記錄任何「例項」度量。這表示使用跨裝置訪客識別時，可能會看到eVar值的「獨特瀏覽」度量大於「例項」度量的報表。
...
* 地域劃分資料會根據瀏覽的第一次點擊進行記錄，而且不會因單次瀏覽而變更，不論使用的裝置為何。 這表示在新裝置上的後續資料連線上，通常不會包含地域劃分資料。
* 技術欄（例如瀏覽器、作業系統和色彩深度）會記錄在第一次瀏覽的點擊上。 如同地域劃分值一樣，它們將不會複製至裝訂的訪客資料。
