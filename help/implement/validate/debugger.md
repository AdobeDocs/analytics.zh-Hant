...
description: 安裝舊版 Adobe Experience Cloud Debugger。 此除錯工具會檢查Analytics、Target、Advertising、Identity Service和資料收集的標籤。
...
>[!IMPORTANT]
>
>這個除錯工具已不再更新。 Adobe 建議改用 [Adobe Experience Cloud Debugger Chrome 擴充功能](https://experienceleague.adobe.com/docs/debugger/using/experience-cloud-debugger.html?lang=zh-tw)。
...
3. 在名稱欄位中，賦予標籤「Adobe Experience Cloud Debugger」，然後將程式碼片段貼入 URL 欄位。
...
2. 按一下整理 > 新書籤。
3. 在名稱欄位中，賦予標籤「Adobe Experience Cloud Debugger」，然後將程式碼片段貼入位置欄位。 標記和關鍵字欄位不是必填欄位。
...
* **易記格式**：將影像要求中的格式顯示方式切換為實用標籤或原始查詢字串。 如需詳細資訊，請參閱[資料彙集查詢參數](query-parameters.md)。
