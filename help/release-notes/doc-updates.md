...
* 如需關於 Adobe 串流媒體服務的資訊，請參閱[在 Analytics 中測量音訊和影片](https://experienceleague.adobe.com/zh-hant/docs/media-analytics/using/media-overview)。
...
| 調整交易 ID 保留期 | 90 天的交易 ID 保留期已延長至 25 個月 交易 ID 變數可唯一識別交易，因此點擊可繫結至透過資料來源上傳的資料。 請參閱[頁面變數](/help/implement/vars/page-vars/transactionid.md)和[資料來源](/help/import/data-sources/transactionid.md)。 |
