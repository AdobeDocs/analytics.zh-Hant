...
| 澄清為資料摘要設定 SFTP 不需要 Adobe 客戶服務 | 在[使用 SFTP 將 Adobe 資料傳送到外部 FTP 帳戶](/help/export/ftp-and-sftp/c-sftp/ftp-sftp-transfer.md)中澄清不需要求助 Adobe 客戶服務即可為資料摘要設定 SFTP。 <p>也新增注意事項，即不再建議使用 SFTP，客戶在設定資料摘要時應使用雲端目的地。</p> |
