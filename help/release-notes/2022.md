...
| **更新支援部分客戶的瀏覽器加密方法** | 2022 年 3 月 28 日 | Adobe 提供兩種密碼安全級別，以滿足不同客戶對第一方數據收集的安全需求。自 **2022 年 6 月 23 日**&#x200B;起，對於安全等級設定為「高」的客戶，特定 HTTPS 加密算法 (稱為加密) 的支援服務就會移除。此操作表示有些較舊版作業系統無法再將資料發送到 Analytics，因為這些系統不支援現代化的加密方法。使用預設「標準」密碼安全設定的客戶不會受到影響。我們已直接聯絡過目前設定為「高」等級的所有客戶。受此變動影響的加密詳細列表可在此處查看。 |
