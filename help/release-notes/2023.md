...
| **更新以下專案的命名慣例： [瀏覽器](../components/dimensions/browser.md) 維度** | 2023 年 1 月 17 日 | 從Chrome、Edge和Firefox瀏覽器的110版開始，公司名稱不再出現在瀏覽器之前。 如果您有根據瀏覽器名稱的區段定義，這可能會影響報表。 例如，從110版開始，包含「瀏覽器包含『Google』」定義的區段將不再識別Chrome瀏覽器。 此外，從Chrome 110開始，行動版和非行動版的Chrome標籤都相同。 之前，行動版本和非行動版本會分別加上標籤。 您可以使用行動裝置細分和劃分，比較行動和非行動裝置流量。 |
