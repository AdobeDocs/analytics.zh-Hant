...
| 通知 | 新增或更新日期 | 說明 |
| ----------- | ---------- | ---------- |
| **Adobe Experience Platform Edge Network的完整IP模糊化** | 2023 年 9 月 27 日 | Adobe計畫在2023年10月更新來自Edge Network之點選的IP模糊化功能。 去年4月，Edge Network新增了模糊化IP位址的功能。 當時，由於Analytics處理來自Edge Network的點選的方式，Adobe Analytics僅支援IP的部分模糊化功能。 客戶為Edge Network選擇完全模糊化時，Analytics只會收到部分模糊化的IP。 實作此變更時，Analytics會收到完全模糊化的IP。 |
...
| 通知 | 新增或更新日期 | 說明 |
| ----------- | ---------- | ---------- |
| **支援接收模糊化的 IP 位址** | 2023 年 9 月 27 日 | Adobe計畫針對來自Adobe Experience Platform Edge Network的點選更新IP模糊化功能。 2023年4月，Edge Network新增了模糊化IP位址的功能。 當時，由於Analytics處理來自Edge Network的點選的方式，Adobe Analytics僅支援IP的部分模糊化。 客戶為Edge Network選擇完全模糊化時，Analytics只會收到部分模糊化的IP。 透過這項變更，Analytics現在會收到完全模糊化的IP。 |
...
| **Web SDK 部分支援 Activity Map** | 從 Web SDK 2.15.0 版本開始，我們開始在啟用連結追蹤時填入 Activity Map 資料。如果 Web SDK 已啟用連結追蹤且 Analytics 已設定 Activity Map，這允許 Web SDK 使用者取得 Activity Map 報告。<p>目前，如果 Web SDK 啟用連結追蹤，在客戶從一個頁面導覽到下一個頁面時會傳送連結事件。這與 AppMeasurement 的運作方式不同，可能會導致傳送額外的計費點擊給 Adobe。在[此處](https://experienceleague.adobe.com/docs/experience-platform/edge/data-collection/track-links.html?lang=zh-Hant)和[此處](/help/analyze/activity-map/activitymap-getting-started/activitymap-enable.md)了解更多資訊 | 不適用 | 2023 年 3 月 31 日 |
| **Adobe Experience Platform Edge Network的IP模糊化** | Edge Network的Adobe計畫可針對直接傳送至Adobe Experience Platform的資料支援IP模糊化功能。 這項變更對於直接將資料傳送至平台，以用於Customer Journey Analytics或其他平台解決方案的客戶有利。 IP 混淆是在資料流層級設定的。它支援移除最後一個八位元或整個 IP 地址。<p>**附註**：混淆不適用於傳送到 Adobe Analytics 的資料。Analytics 會繼續獲取完整的 IP。IP 處理會繼續在 Analytics 中單獨完成。Adobe計畫在未來允許Edge Network層級的Analytics資料模糊化。 | 不適用 | Adobe Experience Platform 於 2023 年 4 月 26 日發行 |
