...
| **管理員設定，控制用於匯出和匯入的帳戶和位置** | Locations Manager中新增的「管理員設定」索引標籤可讓管理員控制使用者是否可以建立和編輯帳戶與位置。 使用者設定雲端匯入和匯出帳戶，以及設定雲端匯入和匯出位置時，這些設定即適用。 <p>管理員也可以限制使用者可以建立和使用的帳戶型別(Google Cloud Platform、Azure RBAC、Amazon S3等)。</p><p>以前，任何使用者都可以為任何型別的帳戶建立、編輯和使用帳戶和位置。</p><p>（更新說明檔案連結，請前往）</p> | 2024 年 6 月 | 2024 年 6 月 |
| **共用用於匯出和匯入的帳戶和位置** | 使用者現在可以將他們建立的帳戶和位置提供給組織中的所有使用者。 只有帳戶和位置擁有者和系統管理員可以編輯和刪除帳戶和位置。<p>以前，帳戶和位置只能由建立它們的使用者使用。</p><p>使用者設定雲端匯入和匯出帳戶，以及設定雲端匯入和匯出位置時，可使用這些設定。 </p> <p>（更新說明檔案連結，請前往）</p> | 2024 年 6 月 | 2024 年 6 月 |
| **Activity Map 可使用更少 Web SDK 伺服器呼叫** | 目前，Activity Map 連結事件被視為是其本身事件並會產生額外費用。此增強功能會產生一些連結事件，並將這些事件封裝至下一個點擊中，類似於 AppMeasurement 處理事件的方式。 <p>（更新說明檔案連結，請前往）</p> | 測試版將於 2024 年 5 月 31 日開始 | 待定 |
