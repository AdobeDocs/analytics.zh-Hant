...
完整處理資料來源歷來可讓組織將點選層級資料提交至Adobe Analytics。 此資料的處理方式與透過傳統資料收集方式(例如AppMeasurement)收集的資料相同。 2020年，Adobe發行了 [大量資料插入API](https://developer.adobe.com/analytics-apis/docs/2.0/guides/endpoints/bulk-data-insertion/)，會執行與完整處理資料來源相同的功能，但具有額外功能。 本頁提供大量資料插入API所提供其他功能的詳細資訊，並概述檔案格式的差異。

2021年3月25日，Adobe禁止建立新的完整處理資料來源連線。 2022年1月31日，所有完全處理的資料服務都已停用。
...
* 為確保訪客連續性和歸因，大量資料插入需要檔案中的列依時間順序排序。 請參閱[訪客群組](https://developer.adobe.com/analytics-apis/docs/2.0/guides/endpoints/bulk-data-insertion/visitor-groups/)以了解跨檔案訪客活動的排序。
* 大量資料插入需要以.gzip格式壓縮的.csv檔案。
* BDIA使用 `timestamp` 而非 `date`.

## BDIA和完整處理資料來源之間的變數比較

大量資料插入已引入下列變數，這些變數先前在完整處理資料來源中無法使用：
...
* **`tnta`**：目標資料裝載使用於 [目標分析](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t.html) 整合。
