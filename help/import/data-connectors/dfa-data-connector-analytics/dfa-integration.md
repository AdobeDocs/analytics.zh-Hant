...
<table id="table_8F6F7F304C36431DA5FD6E5D54F60FC0"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col3"> Genesis會在報表套裝的「作用中整合清單」中顯示的整合名稱。 </td> 
  </tr> 
  <tr> 
   <td colname="col1"> 1 </td> 
  ...
  <tr> 
   ...
   <td colname="col3"> 選取會接收 DFA 點按量度資料的自訂事件。點按次數表示訪客點按廣告的次數，由DFA的重新導向測量。 點按量度會與 Analytics 點進量度相關連。 <p>Note:  DFA Clicks and Analytics Click-throughs might not match exactly due to differences in the way data is collected.  </a>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col3"> 接收 DFA 閱覽量度資料的自訂事件。使用包含檢視變數的「檢視次數」事件來查看哪些促銷活動不會影響直接點進，但可能在後續某個時間對網站流量造成影響。 <p>資料連接器將選取的自訂事件重新命名為「檢視次數」。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col3"> <p>會在每次  <span class="varname"> s.maxDelay </span> timer expires, and no response was received from the DFA servers. Use this event to configure the <span class="varname"> s.maxDelay </span> variable Tuning s.maxDelay </a>.) </p> </td> 
  </tr> 
 </tbody> 
</table>
...
雖然並不常見，但有些網域已達到部分網路瀏覽器的 cookie 容量上限。為避免影響訪客在您網站上的瀏覽體驗，請洽詢您的網路營運、開發團隊或工程群組，以確認將其他Cookie新增至用於DFA整合之頁面的網域不會影響使用者體驗。 您將必須選取 cookie 的名稱。
...
若想瞭解何時應從您 DFA 廣告促銷活動的 DFA 資料中要求閱覽和點進資料，Data connectors 必須識別出訪客在何時點按了 DFA 促銷活動橫幅廣告。若要實現此目的，您必須將區分的查詢字串參數新增至DFA廣告促銷活動的著陸頁面URL，讓「資料連接器」能夠區分DFA廣告促銷活動頁面和您網站上可能擁有的其他廣告促銷活動頁面。 用 `dfa_overrideParam` 於DFA的JavaScript外掛程式中的。

>[!CAUTION]
>
>雖然「促銷活動」變數可用於其他促銷活動，但請勿將其用於DFA促銷活動。 如果您將促銷活動變數設定為 DFA 促銷活動著陸頁面，Adobe 會無法將曝光數與點按繫結至 DFA 促銷活動點進。在每次造訪時，Adobe 收集伺服器都會在 DFA 伺服器中檢查是否有先前的點按或閱覽。因此，請僅在常見登陸頁面上加入DFA外掛程式程式碼，以避免可能拖慢頁面載入時間的不必要重新導向，尤其是對於網際網路連線較慢的使用者。
...
> [!NOTE] 在Google DFA的最新版本中，「精選」一詞已變更為Floodlight。 JavaScript 參數 `dfa_SPOTID` 是根據 Spotlight 術語而命名的，但用於兩個版本中。
...
**maxDelay**: 指定 JavaScript 資料收集代碼等待 DFA Floodlight 伺服器發出回應的時間長度，以毫秒為單位。Adobe建議您嘗試使用此值，以根據您網站的流量找出最佳值。 例如，增加此值通常可收集到較多 DFA 資料，但也會提高失去基本訪客資料的風險 (如果訪客在延遲時段內離開網站)。降低此值可減少失去點擊資料的風險，但可能也會減少與 Adobe 點擊資料一起傳送的 DFA 資料量。
...
一般而言，提高或降低訪客數 *`s.maxDelay`* 量的決定需在取得更多DFA訪客資料與危及Adobe訪客資料收集之間做出權衡。 Increasing *`s.maxDelay`* obtains more DFA visitor data, but (placed excessively high) could endanger the collection of Adobe visitor data. 降低 s.maxDelay 可確保能夠收集 Adobe 訪客資料，但可能會失去 DFA 訪客資料。

*`s.maxDelay`*&#x200B;所包含的不僅止是連絡 DFA 時的網路通訊時間；它也代表瀏覽器在引發及評估這些通訊所依據之 JavaScript 時的延遲。這是因為Integrate模組在將HTML元素插入 *`s.maxDelay`* DOM（會從DFA Floodlight伺服器提取資料）後，就會開始計時器。 瀏覽器根據這個新的 HTTP 元素起始 HTML 要求實際花費的時間量，會隨著同時載入的其他影像或 JavaScript 檔案、訪客電腦的速度和特定的瀏覽器實施方式而有所不同。此外，在從 DFA Floodlight 伺服器擷取 JSON 資料時，瀏覽器必須要評估 JavaScript。這項作業同樣也完全由瀏覽器所控制，且若有大量的 JavaScript 代碼同時執行，或是有許多非同步的 JavaScript 要求，此作業則可能延遲。
...
在調整 *`s.maxDelay`*, because it is incremented every time the s.maxDelay timeout is reached. 在決定是否要增加或減少時，我們 *`s.maxDelay`* 建議遵循此程式：

1. 收集設為特定值 *`s.maxDelay`* 的數天資料。
