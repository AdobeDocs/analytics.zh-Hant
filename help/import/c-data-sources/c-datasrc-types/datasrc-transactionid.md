---
title: 交易 ID 資料來源
description: 了解使用交易 ID 資料來源的一般工作流程。
feature: Data Sources
exl-id: 5f26b15c-8d9c-46d5-860f-13fdfa21af2e
source-git-commit: 79294cfc6f86e5a41a39504099cd730f53668725
workflow-type: tm+mt
source-wordcount: '0'
ht-degree: 0%

---

# 交易 ID 資料來源

交易 ID 資料來源不僅可讓您並排檢視線上和離線資料，還可將資料關聯在一起。若要使用此功能，請在 Analytics 實作中使用 [`transactionID`](/help/implement/vars/page-vars/transactionid.md) 變數。

當您傳送包含 `transactionID` 值的線上點擊時，Adobe 會對當時所有已設定或持續存在的變數拍攝「快照」。如果找到透過 Data Sources 上傳的相符交易 ID，離線和線上資料就會繫結在一起。

為了使用事務，必須先發送並處理具有事務ID的聯機命中，然後才能發送具有該事務ID的任何事務資料源資料。 聯機命中包含變數（eVars等），但不包含事件，這些變數是使用事務ID資訊保存的聯機命中。

當事務資料源命中被發送時，資料源事務命中的事務ID查找變數等。 （不是事件），它們與與該事務ID的原始聯機命中關聯。 如果在資料源事務命中上傳遞的變數沒有值，則在資料源事務命中上使用這些變數。

## 範例

如果傳入並傳遞了事務ID為1256的聯機命中 `evar1=blue`。 `evar2=water` 和 `event1` 設定，然後保存事務ID 1256的事務資料， `evar1=blue`。 `evar2=water`。 不會將事件值保存為事務資訊的一部分。

現在，假設資料源事務命中隨後通過事務ID為1256的系統， `evar1=yellow`。 `evar3=mountain` 和 `event2` 的下界。 系統在資料源事務命中集中查找保存的事務資料 `evar2=water` （因為這是原版的劇集）。 未設定 `evar1=blue` （因為它在原始點擊中） `evar1` （黃色）已在資料源事務命中設定。  因此資料源事務命中導致 `evar1=yellow`。 `evar2=water` （源於原始線上點擊） `evar3=mountain`。 這3個eVar值 `event2` set — 資料源事務命中的事件。

沒有從資料源事務命中獲取值 `event1` 在處理資料源事務命中時設定。

## 交易 ID 資料來源的整體工作流程

使用以下一般工作流程開始使用交易 ID 資料來源：

1. 建立資料來源 (「一般」類別和「一般資料來源 (交易 ID)」類型)。
1. 按照資料源設定嚮導獲取FTP位置以上載資料並下載資料源模板檔案。
1. 更新您的實作，加入 `transactionID` 變數。
1. 使用 `.fin` 檔案將資料來源檔案上傳至 FTP 站台。

## 上傳檔案和實作程式碼範例

如果您上傳以下資料來源檔案，並在站台上實作以下程式碼，報表中就會顯示連結的資料。資料來源檔案使用 eVar1 和 event1，線上實作則使用 eVar2 和 event2。由於交易 ID 相符，您可以看到 eVar1 的 event2 資料，以及 eVar2 的 event1 資料。

### 範例檔

下載範本、更新值，然後將其上傳至資料來源 FTP 位置：

| `#` | `Example eVar1 name` | `Example event 1 name` | `1` |
|---|---|---|---|
| `Date` | `Evar 1` | `Event 1` | `transactionID` |
| `01/01/2020/12/00/00` | `Example eVar1 value` | `1` | `1234` |

### 實作程式碼範例

如需交易 ID 的詳細說明，請參閱實作使用者指南中的 [`transactionID`](/help/implement/vars/page-vars/transactionid.md)。

```js
var s = s_gi("examplersid");
s.eVar2 = "Example eVar2 value";
s.events = "event2";
s.transactionID = "1234";
s.t();
```
