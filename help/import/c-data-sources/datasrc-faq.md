...
完全處理僅支援舊式事件清單格式，但不包括直接在事件清單中的數值／貨幣／計數器（超過1個）事件值，這並 `"eventNN,eventKK"` 非 `"eventNN=#.##"`。 這表示，只有當計數器事件傳入資料來源檔案中的events欄，且遞增1時，它才支援該計數器事件。

如果需要數值、貨幣或計數器（超過1個）事件，請使用產品清單：
