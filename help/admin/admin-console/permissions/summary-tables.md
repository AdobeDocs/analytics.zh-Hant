...
| 資料摘要管理器 | x |  |  |  | 授與Analytics頂端導覽中「管理員>資料饋送」的存取權。 |
| 資料修復 API | x |  |  |  | 授予資料修復 API 的權限 |
| 資料來源管理器 | x |  |  |  | 授與Analytics頂端導覽中「管理員>所有管理員>資料來源」的存取權。 需要權限項目「報告套裝管理」。 |
...
| --- | --- | --- | --- | --- | --- |
| 代碼管理器 | x |  |  |  | 保留供舊版使用。授與「管理員>所有管理員>代碼管理員」的存取權。 在多數情況下，請改用 Adobe Experience Platform Launch 中的資料庫。 |
| 代碼管理器 - 網站服務 | x |  |  |  | 透過 1.4 API 授予代碼管理器的存取權。 |
| 隱藏報表套裝 | x |  |  |  | 授與「管理員>所有管理員>公司設定>隱藏報表套裝」的存取權。 授予隱藏組織中任何報告套裝的能力，不論組織對報告套裝是否具備存取權。 |
| 整合 (建立) | x |  |  |  | 授與「管理員>所有管理員>資料連接器」的存取權，並允許使用者建立資料連接器。 |
| 整合 (刪除) | x |  |  |  | 授與「管理員>所有管理員>資料連接器」的存取權，並允許使用者刪除資料連接器。 |
| 整合 (更新) | x |  |  |  | 授與「管理員>所有管理員>資料連接器」的存取權，並允許使用者修改現有的連接器組態。 |
| 記錄檔 | x |  |  |  | 授予「管理員>所有管理員>記錄檔」的存取權。 |
| 記錄檔 - 網站服務 | x |  |  |  | 透過 1.4 API 授予提取記錄檔的存取權。 |
| 擱置中的動作 | x |  |  |  | 授予「管理員>所有管理員>公司設定>待定動作」的存取權。 |
| 安全性 | x |  |  |  | 授與「管理員>所有管理員>公司設定>安全管理員」的存取權。 |
| 伺服器呼叫使用量 | x |  |  |  | 授與「管理員>伺服器呼叫使用」的存取權。 |
| 支援 | x |  |  |  | 授予「管理員>所有管理員>公司設定>支援資訊」的存取權。 |
| 流量管理 | x |  |  |  | 授予「管理員>所有管理員>流量管理」的存取權。 |
| 網路服務 | x |  |  |  | 授與「管理員>所有管理員>公司設定>網站服務」的存取權。 |
...
| Excel 授權用戶 |  |  |  | x | 未使用。 |
| 行動應用程式管理員 |  |  |  | x | 未使用。 |
| 權限管理 |  |  |  | x | 已不再使用。授與「管理員>所有管理員>使用者管理」下舊版使用者管理介面的存取權。 |
