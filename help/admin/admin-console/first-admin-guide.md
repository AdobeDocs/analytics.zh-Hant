...
Adobe Analytics支援各種類型的管理員。 完整的Adobe Analytics管理員可完整存取Adobe Analytics中的所有項目，而其他管理員和使用者則可執行更專業的工作。
...
* **Experience Cloud第一管理員：** 第一個管理員是讓組織其他成員使用每個Experience Cloud解決方案的起點。 簽署合約後，Adobe的布建團隊會準備建立帳戶。 第一位管理員會在合約開始日期之前收到含有登入憑證的電子郵件。強烈建議您務必先將第一位管理員的聯絡資訊提供給 Adobe，且確保資訊正確無誤，再簽署合約。
