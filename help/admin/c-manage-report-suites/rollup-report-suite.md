...
* 統計可提供資料總計，但不會在報表中報告個別值。 例如，eVar1值不包含，但其匯總總計可以包含。
* 在結合各報表套裝的資料時，不會進行去重複化。
* 系統於每晚執行統計。
* 新增報表套裝至已經存在的統計時，歷史資料並不包含在統計中。
* 所有子報表套裝都必須包含資料，才能讓統計運作。 如果統計中包含新的報表套裝，請務必至少傳送一個頁面檢視給這些報表套裝。
* 統計報表套裝最多可限制為40個子報表套裝。
* 統計報表套裝最多可以有100個事件。
...
**複製**：全域報表套裝會複製獨特訪客，但是統計不會。例如，如果使用者在同一天瀏覽了您的三個網域，統計會計算三個每日獨特訪客。 全域報表套裝會記錄一個獨特訪客。
