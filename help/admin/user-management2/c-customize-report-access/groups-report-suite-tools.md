...
**[!UICONTROL 使用者管理]** &gt; **[!UICONTROL 群組]** &gt;報 **[!UICONTROL 表存取]** &gt;報 **[!UICONTROL 表套裝工具]****[!UICONTROL &gt;自訂]**
...
| Data Warehouse | 允許非管理員使用者透過網站服務API使用資料倉庫方法進行呼叫。 See [Data Warehouse - Developer Documentation](/help/export/data-warehouse/data-warehouse.md) |
...
| [即時報告設定](/help/admin/admin/realtime/t-realtime-admin.md) | 設定即時報告分析的權限。 |
...
| [尋找方法](/help/admin/admin/finding-methods.md) | 可讓您識別不同的尋找方法報告如何接收網站上轉換成功事件的評價。 |
...
| [即時報表](/help/admin/admin/realtime/t-realtime-admin.md) | 授予對即時報告的存取。 |
| [機器人頁面](/help/admin/admin/bot-removal/bot-rules.md) | 機器人規則可讓您移除報表套裝中，由已知編目程式和機器人產生的流量。移除機器人流量可以提供網站上更準確的使用者活動測量。 |
| [Bot](/help/admin/admin/bot-removal/bot-rules.md) | 機器人規則可讓您移除報表套裝中，由已知編目程式和機器人產生的流量。移除機器人流量可以提供網站上更準確的使用者活動測量。 |
...
| 分類 | 結合下列權限：「流量分類」、「視訊分類」、「轉換分類」、「分類階層」、「分類管理員」和「分類匯入／匯出器與規則產生器」。  注意: 透過此權限，使用者可編輯所有報表套裝的分類，而不僅僅只有選取的套裝。 |
...
| **控制面板項目** |  |
| 「控制面板項目」中的設定可讓您存取「報告與分析 [](https://marketing.adobe.com/resources/help/en_US/sc/user/dashboard.html) 」中的小報表、我的建議報表、公司摘要報表、影像、KPI/量規報表、報表套裝總計、文字、小報表、使用摘要報表和網頁資源 |  |
| **其他** |  |
