...
「**[!UICONTROL 資料規範的隱私標籤]**」對話框概觀了報表套裝的隱私標籤和命名空間。 您還可以從此處將設定匯出至 .csv 檔案。

## 檢視隱私權標籤 {#view-privacy}

1. 登入Adobe CX Enterprise。
2. 導覽至 **[!UICONTROL Analytics]** > **[!UICONTROL 管理員]** > **[!UICONTROL 所有管理員]** > **[!UICONTROL 資料設定和收集]** > **[!UICONTROL 資料控管]**。

   >[!NOTE]
   >
   >如果並未看到此選單項目，則需要將您新增至 [Admin Console 的產品設定檔](/help/admin/admin-console/permissions/product-profile.md)，並提供您使用此功能的權限，或是已提供在 Admin Console 中報告套裝的存取權。
...
1. 最後產生的螢幕會顯示變數名稱、您嘗試複製且目前套用的標籤、報表痮裝及其 ID，以及目標報表套裝中的設定是否匹配。
   ...
   >[!IMPORTANT]
   >
   >請記住，您選取的所有報表套裝都必須對應至您的CX Enterprise組織。

   將一個變數或一組變數的標籤複製到不同的報告套裝時，複製的標籤會出現在目標報告套裝中對應位置的變數。 若是標準元件、清單變數、成功事件，系統會將標籤複製到目標報表套裝中&#x200B;**名稱相同**&#x200B;的變數。

   但是，對於轉換變數 (eVar) 和流量維度 (prop)，系統會將標籤複製到目標報告套裝中&#x200B;**數字相同**&#x200B;的變數。 例如，eVar12將會複製到所有目標報表套裝的eVar12中。 在決定複製目標時，將會忽略這些變數的名稱。 如果目的地報表套裝中未啟用對應的變數，該變數的複製將會失敗。

   復製為變數定義的分類標籤時，標籤會複製到目標報表套裝中對應變數上的分類（例如eVar7至eVar7），其名稱與要複製的分類相同。 否則，該分類標籤的複製作業將會失敗。
...
參考[指派或編輯報表套裝標籤](/help/admin/tools/privacy-labeling/labeling-overview.md)。
