...
| I2 | 可間接識別：可與任何其他資料合併使用，以識別或直接聯絡個人或裝置的資料，Does not allow identification of an individual by itself, but can be combined with other information (that may or may not be in your possession) to identify someone. 例如：客戶忠誠度編號或公司 CRM 系統所用的 ID (每位客戶指定一個唯一 ID)。 | <ul><li>無法在事件上設定</li><li>無法在銷售eVar上設定</li></ul> |
...
只有在欄位包含允許點擊與資料主體建立關聯的值時 (亦即允許辨識資料主體的身分)，才需要使用刪除標籤。 因為與資料主體的關聯將會截斷，則其他個人資訊 (最愛、瀏覽/購買歷史記錄、健康情況等) 無需刪除。
...
| <ul><li>流量變數(Prop)</li><li>Commerce variables (non-merchandising eVars)</li></ul> | 所有標籤 | - |
