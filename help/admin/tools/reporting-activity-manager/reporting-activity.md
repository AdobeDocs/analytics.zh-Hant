...
1. 檢視有關每個報告套裝的使用情況資訊。 表格中顯示的資料代表頁面上次載入時報告套裝的報告活動。
   ...
   | **[!UICONTROL 虛擬報表套裝]** | 顯示所有匯入此基本報告套裝的虛擬報告套裝。 虛擬報告套裝因套用了額外層級的篩選和細分，而增加了報告請求的複雜性。 來自虛擬報告套裝的所有請求均會合併為基本報告套裝。 |
   | **[!UICONTROL 容量使用情況]** | 即時顯示已使用報告套裝的報告容量百分比。 <p>**請注意** 使用容量達到 100%，不一定表示您應該立即開始取消報告請求。 如果平均等待時間合理，100% 的使用容量可能屬於健康狀態。 另一方面，如果已佇列的請求數量也在增加中，則 100% 的使用容量可能表示有問題。</p> |
...
請求圖表顯示過去 2 小時內選取的報告套裝已處理和佇列中的請求數量。

將滑鼠停留在圖表上，即可檢視該分鐘請求數量達到最高值的時間點。
...
| 欄 | 說明 |
| --- | --- |
| [!UICONTROL **請求 ID**] | 可用於疑難排解目的的唯一識別碼。 若要複製 ID，請選取此請求，然後選取「[!UICONTROL **複製請求 ID**]」選項。 |
...
| [!UICONTROL **專案**] | 儲存的Workspace專案名稱、API報表ID等。（中繼資料可能因各種應用程式而異。） |
| [!UICONTROL **狀態**] | 狀態指示器： <ul><li>**執行中**：請求目前正處理中。</li><li>**擱置中**：請求正等待處理中。</li></ul> |
| [!UICONTROL **複雜度**] | 並非所有請求皆需要相同的時間進行處理。 請求複雜度可協助提供有關處理請求所需時間的一般概念。 <p>可能的值包括：</p> <ul><li>[!UICONTROL **低**]</li><li>[!UICONTROL **中**]</li><li>[!UICONTROL **高**]</li></ul>此值受下列欄中的值影響：<ul><li>[!UICONTROL **月邊界**]</li><li>[!UICONTROL **欄**]</li><li>[!UICONTROL **區段**]</li></ul> |
| [!UICONTROL **月邊界**] | 請求中包括的月份數。 更多月邊界會增加請求的複雜度。 |
| [!UICONTROL **欄**] | 請求中的量度和劃分的數量。 更多欄會增加請求的複雜度。 |
| [!UICONTROL **區段**] | 套用至請求的區段數。 更多區段會增加請求的複雜度。 |
...
| [!UICONTROL **平均月邊界**] | 請求中包括的平均月份數。 月邊界越多，請求的複雜度就越高。 |
| [!UICONTROL **平均欄數**] | 請求中包括的量度和劃分的平均數量。 更多欄會增加請求的複雜度。 |
| [!UICONTROL **平均區段數**] | 套用於所含請求的平均區段數。 更多區段新增請求的複雜度。 |
...
| [!UICONTROL **平均月邊界**] | 請求中包括的平均月份數。 月邊界越多，請求的複雜度就越高。 |
| [!UICONTROL **平均欄數**] | 請求中包括的量度和劃分的平均數量。 更多欄會增加請求的複雜度。 |
| [!UICONTROL **平均區段數**] | 套用於所含請求的平均區段數。 區段越多，請求的複雜度就越高。 |
