...
轉換分類可讓您分類轉換變數。 分類之後，任何您可以使用關鍵資料產生的報表，也可以使用關聯的資料屬性來產生。

>[!WARNING]
>
>重新命名分類可能會導致「[分類規則生產器](/help/components/classifications/crb/classification-rule-builder.md)」中建立的現有規則發生問題。 如果您重新命名具有分類規則的分類，請確定您修正每個規則，使其指向已重新命名的分類。
...
| 數值類型 (僅限數值) | 指定數值分類中的數值類型。 選項包括數值、百分比與貨幣。 |
...
1. 在下拉式清單對話方塊中新增或移除選項。
   ...
   新增選項會建立可用於此分類的分類值清單。 您可以使用這個選項搭配行銷活動變數，在「行銷活動管理員」裡提供用戶一份受支援分類值的清單。 當分類維度中有少數允許的值很少或永遠不會變更時，請使用此選項。 例如您可能會針對不同等級的客戶忠誠度進行不同的行銷活動：銀級、金級和白金級。 然後，您可以使用下拉式清單，確保只接受符合您三個層級的值。 如果有人嘗試使用不同的值，則會捨棄該值。
