---
title: 行銷管道中的分類
description: 使用分類將值分類為相關群組，並在群組層級報告。 例如，您可將所有付費搜尋行銷活動都分類到「視訊卡」之類的類別中。 然後您可以報告該類別相對於「例項」（或「點進次數」）等量度的成功狀況。
...
使用分類將值分類為相關群組，並在群組層級報告。 例如，您可將所有付費搜尋行銷活動都分類到「視訊卡」之類的類別中。 然後您可以報告該類別相對於「例項」（或「點進次數」）等量度的成功狀況。
...
| 詞語 | 定義 |
|--- |--- |
| 文字分類 | 指定分類類別的型別，讓您將變數資料分組，以用於報表用途。 |
