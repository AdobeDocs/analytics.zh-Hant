...
使用虛擬報表套裝來取代多套裝標記通常是最佳做法，但虛擬報表套裝有一些限制。 請參閱「[虛擬報表套裝和多套裝標記考量事項](/help/components/vrs/vrs-considerations.md)」，以判斷哪一種報表套裝做法是符合貴公司需求的最佳選擇。 有關虛擬報告套件和多套件標籤功能的深入比較，請參閱[虛擬報告套件與多套件標籤](/help/components/vrs/vrs-about.md)。
