...
機器人規則可讓您移除由已知編目程式和機器人程式產生的報表套裝流量。 移除機器人流量可以提供網站上更準確的使用者活動測量。
...
機器人流量資料儲存在單獨的儲存庫中，以便顯示在  機器人和機器人頁面報告中。啟用機器人篩選有兩個選項：
...
勾選「啟用IAB機器人篩選規則」核取方塊，即可 [!UICONTROL 開啟標準IAB機器人規則] 。 此選擇將移除IAB（國際廣告局）國際蜘蛛程式與機器人清單中的機器人程式，以移除機器人流量。 IAB會每月更新此清單。

![](assets/bot-iab-checkbox.png)

Adobe 無法提供詳細的 IAB 機器人清單給客戶，但您可使用「機器人報告」來檢視曾存取您網站之機器人的清單。若要將機器人送出至IAB清單，請造訪 [IAB](https://www.iab.com)。
...
* **單一IP位址或萬用字元符合**:若要比對單一IP位址( `10.10.10.1`)或萬用字元IP位址( `10.10.*.*`)，請在「IP開始」和「IP結束」欄中放置相同的值。 「符合規則」、「代理包含」和「代理排除」必須為空。
