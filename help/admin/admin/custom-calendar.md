...
>[!CAUTION]
>
>若變更日曆，將連帶變更資料的處理方式 (例如每週與每月不重複訪客的定義)。當日曆在週與月的定義上有所變更時，歷史資料仍維持不變。此設定還會根據日期範圍影響段。
...
>[!NOTE]
>除Data Warehouse外，所有Adobe Analytics工具(Analysis Workspace、報告和分析、Report Builder、Activity Map)都支援這些日曆選項。 Data Warehouse完全支援陽曆。 在選擇非西曆日曆時，Data Warehouse將使用非西曆日曆的預期日期範圍，但報告行中的日/周/月細分可能不是非西曆日曆中預期的。
