...
本非常詳細的檔案說明銷售eVar背後的概念，這些eVar的處理和配置資料的方式與標準eVar不同。 也說明銷售eVar與產品尋找方法的關聯。

## 概觀

使用銷售 eVars 可以讓您將任何成功的活動分配到從&#x200B;*每個產品*&#x200B;層級上的 eVar 所擷取的數值，而不是從&#x200B;*每次造訪/每次訂購*&#x200B;的層級上所擷取的數值。
...
我們可以使用額外的 eVar 來衡量相互關係中所有產品尋找方法的性能。除了上述尋找方法之外，eVar在比較中也包含其他尋找方法，例如從外部網站連結至產品詳細資料頁面。

* eVar1：產品尋找方法

將這些變數設為銷售eVar，而非標準eVar，而是將其設為銷售eVar。
...
以下是您可搭配銷售eVar使用的不同設定。 以下螢幕擷取來自「報表套裝管理器」。 前往「[!UICONTROL Analytics] > [!UICONTROL 管理員] > [!UICONTROL 報表套裝] > [!UICONTROL 編輯設定] > [!UICONTROL 轉換] > [!UICONTROL 轉換變數] > [!UICONTROL 新增] > [!UICONTROL 啟用銷售A15/」，即可存取。]

![](assets/merch-evars1.png)

請在下表各節中，找到有關這些設定的詳細資訊。
...
標準eVar無法使用此選項。 [!UICONTROL 銷售]設定可讓您選擇[!UICONTROL 轉換變數語法]或[!UICONTROL 產品語法]作為擷取銷售eVar值的方法。
...
如前所述，所有具有轉換變數語法的銷售eVar只有「最近（最後）」配置。 因此，銷售eVar的配置設定不會決定當訪客繼續使用網站時，要插入哪些值到post_evar欄中。 相反地，此設定會決定哪些eVar值會系結至產品，以及這類產品如何將其成功事件配置回其系結的eVar值。

當銷售eVar的配置（即系結）設定設為等於「原始值（第一個）」時，會發生下列情況：任何與post_evar欄一併設定，且先前未系結至post_evar欄之對應「預先處理」eVar的產品，都會系結至post_evar欄中包含的值。  根據報表套裝設定中的「過期時間」設定，eVar值和產品之間的此系結在eVar過期之前不會變更。
...
* 當將eVar傳入影像要求後，不應再自動填入post_evar欄時。

eVar的過期時間會在成功事件記錄或一段時間過後發生。 Adobe Analytics一次只允許一個過期設定，每個eVar。

若為「產品尋找方法」，設定銷售eVar有效期的最佳實務是將其設為等於

* 網站自動從購物車中移除產品（例如14天、30天等）之前，產品在網站購物車中停留的時間
* 或購買事件發生時。

透過任一設定，訪客購買的任何產品都會將訂單/單位/收入評分分配給當時產品系結的銷售eVar值。

### 類型

eVar類型設定決定要插入eVar的資料類型。 在大多數情況下，此值應等於「Text」。 很少對銷售eVar使用「計數器」。 不過，「計數器」可用來根據每項產品將成功分配給計數器eVar值。  使用「計數器」類型討論解決方案不在本檔案的討論範圍內。

### 銷售捆綁事件

「銷售捆綁事件」設定可讓您指定產品要系結至銷售eVar值的條件。 這些條件僅限於特定成功事件或eVar的引發。 觸發流量變數（例如prop）對銷售系結沒有影響。

請注意，「銷售捆綁事件」設定可透過多個事件將產品系結至eVar值。 範例：

* 透過產品檢視事件
* 透過購物車新增事件
* 透過購買事件

依預設，每當有其他事件/eVar（銷售或標準）包含在與產品相同的影像要求中時，設定會將產品系結至銷售eVar值。
...
在許多可用的設定組合中，您可能會想：哪些設定是最佳實務？

如果您想要將「內部關鍵字搜尋」系結至產品ID 12345，產品變數的設定如下：

`s.products=";12345;;;;eVar1=internal keyword search";`

與productID 12345同時擷取的任何成功事件（購物車新增、購買），都會計入產品ID 12345和「內部關鍵字搜尋」的eVar1值。 不同eVar1值接收與產品ID 12345相關聯之成功事件評分的唯一方式，是eVar1之後在產品變數內設為不同值時(與產品ID 12345一併設定)。
...
此變數設定會將產品ID 12345的系結從「內部關鍵字搜尋」的eVar1值變更為「內部促銷活動」的eVar1值。 此外，當eVar設定為使用產品語法和配置（捆綁）設定「最近（最後）」時，會發生此重新綁定更改。 如果配置（捆綁）設定改為設為「原始值（第一個）」，將eVar1設為等於產品ID 12345的「內部促銷活動」將不會將產品ID 12345重新系結至「內部促銷活動」的eVar1值。 相反地，系結將會保留在原本界限的值「內部關鍵字搜尋」。
...
* 從尋找方法頁面將產品尋找方法詳細資訊（如上所示）傳遞至產品詳細資料頁面，以及
* 從上一頁剛傳遞的項目重新建立相同的產品變數值。
...
例如，許多服裝產品都有「子SKU」，會指定大小、顏色、樣式和任何其他屬性。 這些屬性會將單一子產品與屬於相同父產品的其他產品分開。 假設您決定購買中藍色T恤，加上大紅色T恤。 假設這兩件襯衫的父產品ID均為「tshirt123」，且`eVar10`已設定來擷取子SKU。 購買確認頁面上設定的變數如下：
...
您可能會使用「最近（最後）」的配置（捆綁）設定來面臨其他問題。 在許多網頁瀏覽體驗中，訪客會「重新尋找」已檢視和/或新增至購物車的產品。 這通常會透過後續造訪，或在客戶決定完成購買前進行。 假設訪客在造訪網站期間，透過關鍵字搜尋「sandals」找到「sandal123」產品。 他們會立即從關鍵字搜尋結果頁面將其新增至購物車。 擷取購物車新增的程式碼將設定如下：
...
現在，假設訪客在這次造訪期間未購買產品，但三天後隨著「sandals123」產品仍在購物車中而返回網站。 訪客想在購買前進一步了解產品。 但訪客並非使用關鍵字搜尋來尋找產品，而是瀏覽整個網站。 他們最後會出現在「womens > shoes > sandals」銷售瀏覽區段，然後「重新尋找」產品。 當他們最終「重新尋找」「sandal123」產品的產品詳細資料頁面時，變數的設定如下（頁面載入時）:
...
由於我們想要在產品互動發生時將「產品尋找方法」eVar值系結至產品（表示已「找到產品」），因此可以安全地假設最常見的「找到產品」互動是產品檢視（訪客前往產品詳細資料頁面時）或購物車新增（訪客直接從產品尋找方法頁面將產品新增至購物車時）。

因此，我們可以選擇這兩個事件(prodView、scAdd)作為「基本」銷售捆綁事件。
以下是當這些系結事件中的一個包含在影像要求中時會發生什麼情況。 任何包含在相同請求中（在產品變數內）且未系結至銷售eVar的產品ID，都會系結至傳入銷售eVar（post_evar欄）的最新值。 當配置（綁定）設定設定為等於「原始值（第一個）」時，將忽略在此原始綁定發生之後重新綁定這些產品的任何嘗試。

### 最佳實務設定

以下是最佳實務設定。 他們可輕鬆實施產品尋找方法，並獲得最佳結果。 Adobe建議用戶端依下列方式設定其每個產品尋找方法銷售eVar（一般）:
...
在post_evar欄中，Adobe處理伺服器會看到此點擊，如下所示：
...
post_products欄中的值可能很熟悉。 向上捲動本檔案，並比較此post_products值和s.products值，如下所示。  請注意， post_products欄是使用產品變數語法來設定！

這表示系結「複製」轉換變數語法eVar值至產品語法變數。 只有當產品變數和系結事件(透過eVar設定設定)包含在相同請求中時，才會發生此複製動作。 此時，post_eVar欄中包含的值會系結至產品。 此捆綁會透過產品語法來表示，如post_products欄中所儲存。

## 銷售eVar、例項量度及Attribution IQ

在Analytics伺服器呼叫中傳送標準eVar時，其post_evar欄中的值一律會取得該呼叫的例項。 例項代表eVar已設為等於影像要求中特定值的次數。

例如，假設`eVar10`是具有[!UICONTROL 上次接觸]歸因的標準eVar。 如果您在任何頁面上設定`s.eVar10="hello world"`，則當Adobe處理點擊時，「hello world」的值會傳遞至post_evar10欄。 每個個別`eVar10`設定的`hello world`例項量度等於「1」。 請記住，當post_evar欄有值時，不一定會記錄例項。 相反地，post_evar欄會決定在記錄例項時，要取得例項的值。

銷售eVar的例項會為eVar收集的值提供歸因。 但只有系結至銷售eVar值的產品同時「互動」時，才會發生此情況。

例如，自行設定`s.eVar1="Internal Keyword Search"`並不會將任何例項量度評分給予「內部關鍵字搜尋」的eVar1值。 此時會記錄一個例項。 不過，除非產品在設定`eVar1`時系結至該「內部關鍵字搜尋」值，否則例項會歸因於「未指定」貯體。 換言之，「內部關鍵字搜尋」的`eVar1`值可以取得例項。 但只有系結至「內部關鍵字搜尋」值的產品出現在相同影像要求的產品變數中，才會發生此情況。
