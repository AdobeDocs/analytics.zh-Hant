...
| 說明 | 事件用途與用法的簡短說明。 |
| 獨特事件記錄 | **每次造訪記錄一次**：繫結特定事件和訪客的工作階段。同一次瀏覽中，特定事件的後續計數會被忽略。就這類事件序列化而言，實作上不需任何變更。<br>**使用事件 ID **：繫結特定事件和自訂 ID。擁有相同事件 ID 之特定事件的後續計數會被忽略。在這類事件序列化作業中，需使用點擊中的自訂 ID 刪除重複數值。請參閱實作使用手冊中的[事件 ID 序列化](../../../implement/vars/page-vars/events/event-serialization.md)。 |
| 參與率 | 為瀏覽中的所有維度值提供完整的歸因評價。 |
