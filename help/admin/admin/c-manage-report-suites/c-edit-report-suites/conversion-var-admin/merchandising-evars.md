...
我們可以使用額外的 eVar 來衡量相互關係中所有產品尋找方法的性能。除了上述的尋找方法之外，eVar 也包含其他尋找方法 (例如至外部網站產品詳情頁面的連結) 以做為比較。
...
如前所述，所有具有轉換變數語法的銷售eVar只有「最近（最後）」配置。 因此，當訪客繼續使用網站時，銷售 eVar 的配置設定不會決定要將哪些值插入 post_evar 欄。 此設定會決定要將哪一個 eVar 值與產品綁定，以及這類產引如何將其成功事件配置回其綁定的 eVar 值。
...
與productID 12345同時擷取的任何成功事件（購物車新增、購買），都會計入產品ID 12345和「內部關鍵字搜尋」的eVar1值。 對於與產品 ID 12345 相關的成功事件，不同 eVar1 值收到點數的唯一方式就是稍後將 eVar1 設定為產品變數中的不同值 (連同產品 ID 12345)。
...
您可能會使用「最近（最後）」的配置（捆綁）設定來面臨其他問題。 在許多網頁瀏覽體驗中，訪客會「重新尋找」已檢視和/或新增至購物車的產品。 這通常是經由後續造訪或是在他們決定完成購買之前進行。 假設訪客在造訪網站期間，透過關鍵字搜尋「sandals」找到「sandal123」產品。 他們立即從關鍵字搜尋結果頁面將此產品加入購物車。 擷取購物車的程式碼將會設定如下：
...
現在，假設訪客在這次造訪期間未購買產品，但三天後隨著「sandals123」產品仍在購物車中而返回網站。 該訪客想要在購買前進一步了解此產品。 該訪客瀏覽整個網站，不是使用關鍵字搜尋來尋找此產品。 他們最後會出現在「womens > shoes > sandals」銷售瀏覽區段，然後「重新尋找」產品。 當他們最終「重新尋找」「sandal123」產品的產品詳細資料頁面時，變數的設定如下（頁面載入時）:
