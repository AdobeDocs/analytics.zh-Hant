...
>[!NOTE]
>
>此 [虛擬報表套裝檔案](/help/components/vrs/c-workflow-vrs/vrs-create.md) 說明如何建立虛擬報表套裝。
...
| 停用多位元組字元支援 | 停用報告套裝的多位元組字元支援。若您停用多位元組字元支援，系統會假設資料採用 `ISO-8859-1` 格式。網頁必須在 charSet JavaScript 變數中指定其字元集。 <p>多位元組字元支援可使用 UTF-8 儲存報告套裝中的字元。系統收到報告時就會將您網頁資料的字元集轉換為 UTF-8 字元集，因此您可以在行銷報告中使用任何語言。聯絡您的Adobe客戶團隊或客戶服務，以變更現有報表套裝的多位元組字元支援。 |
