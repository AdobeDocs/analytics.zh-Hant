...
| 行銷管道處理規則 | 您可以使用處理規則來準備資料，以供[「行銷管道處理規則」](https://experienceleague.adobe.com/docs/analytics/components/marketing-channels/c-rules.html?lang=zh-Hant)處理。 |
...
如果已將VISTA規則配置為將命中複製到另一個報告套件，則這些命中將通過在另一個報告套件中定義的任何處理規則發送。
