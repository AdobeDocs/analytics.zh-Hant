...
Adobe Audience Manager區段資料是以逐次點選的方式傳入Analytics，並會在該時間點即時表示訪客的對象成員資格。 也就是說，訪客可能會先歸入某個區段 (例如「察覺」)，之後再歸類到更符合的區段 (例如「考慮」)。您可以使用 Analysis Workspace 中的[「流量」](/help/analyze/analysis-workspace/visualizations/fallout/fallout-flow.md)，以視覺化的方式呈現訪客在對象之間所採取的歷程。
...
對象也可用於[流失視覺效果](/help/analyze/analysis-workspace/visualizations/fallout/fallout-flow.md)中。

## Analysis Workspace 中的文氏圖表視覺效果 {#venn}

[文氏圖表視覺效果](/help/analyze/analysis-workspace/visualizations/venn.md)可顯示最多 3 個區段之間的重疊部分。
