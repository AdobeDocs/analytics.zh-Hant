...
1. **設定您的資料保留原則。** Adobe需要資料保留政策才能服務資料隱私權資料存取/刪除請求。  如需詳細資訊，請參閱[資料保留常見問題集](/help/technotes/data-retention.md)。 為了使用 Privacy Services API，您必須確保在 Adobe Analytics 中設定資料保留期。
