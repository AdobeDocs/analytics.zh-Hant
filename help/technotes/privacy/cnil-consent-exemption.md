...
* 地理位置參數。 地理位置的精確度不得高於郵遞區號層級。 [郵遞區號選項](/help/implement/vars/page-vars/zip.md)和[一般帳戶設定](/help/admin/tools/manage-rs/edit-settings/general/general-acct-settings-admin.md)
...
* 存取和刪除能力。 利用 Privacy Service 處理存取和刪除請求。 [Analytics 及 Privacy Service](gdpr.md)

## 資料收集的其他注意事項

以下其他注意事項適用：

* Adobe Analytics 在美國、英國和新加坡經營資料處理中心，以便為所有客戶提供在區域中收集、處理和儲存資料的靈活性。 在配置 Adobe Analytics 的初始設定時，客戶可以選取他們想要的資料處理中心地點。 客戶的資料最終會儲存在他們為核心 Analytics 產品所選取的區域內。
* 請考慮收集 Analytics 變數中的選擇加入狀態，以便針對分段、虛擬報告套裝或路由傳送到個別端點來區分選擇加入的資料與選擇退出的資料。
* 未經事先同意，不得在網站或應用程式外部進行任何測量，例如，不進行場外活動、電子郵件活動或 iFrame。
* 未經同意，不允許在變數中收集個人資訊。 [根據使用者同意控制CX Enterprise活動](https://experienceleague.adobe.com/docs/id-service/using/implementation/opt-in-service/use-opt-in-to-control-experience-cloud-activities-based-on-user-consent.html?lang=zh-Hant#implementing-opt-in-on-the-page)
...
>[!IMPORTANT]
>
>本文件並不是要做為法律或監管建議，也不構成 Adobe 的任何保證或契約承諾。 我們鼓勵客戶就與此主題相關的自身法律和監管義務尋求獨立的法律建議。
