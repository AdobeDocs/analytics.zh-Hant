...
* 13 個月的 Cookie 限制。  您可以使用 `cookieLifetime` 變數覆寫 Analytics Cookie 期限。 Experience Cloud Cookie (包括 Analytics 和 ECID) 會隨著每次瀏覽延長 Cookie 期限。  若要設定靜態、非滾動的Cookie有效期，您可以：(1)撰寫自訂程式碼，設定刪除Cookie的日期，或(2)使用您的CMP控制Cookie重設的日期。  [cookieLifetime](/help/implement/vars/config-vars/cookielifetime.md)和[Experience Cloud Cookie](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-privacy.html?lang=zh-Hant#ec-cookies)
