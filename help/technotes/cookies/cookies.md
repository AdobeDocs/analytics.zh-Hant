...
Adobe 的第一方 Cookie 限制為 7 天到期，或者，如果 Apple 判斷點進來自追蹤器，則為 24 小時到期。 如果是 7 天期限，則當使用者造訪您的網站然後在七天內回訪時，Cookie 的過期日會再延長七天。 但如果使用者造訪您的網站然後在八天後回訪，則會在第二次造訪時將其視為新使用者。

目前，ITP政策適用於Adobe設定的所有第一方Cookie，無論您是使用訪客ID服務或舊有的Analytics ID (&quot;s_vi&quot; Cookie)。 一方面，這些政策僅適用於 Cookie 設定的用戶端，而不適用於 Cookie 設定的伺服器端 (透過 CNAME 實作)。 但在 2020 年 11 月，ITP 已更新為同樣適用於 CNAME 實作。
...
* `None`：此設定可啟用跨網站存取，以及啟用在第三方內容中傳遞 Cookie。 若要指定此屬性，您也必須指定 `Secure`，且所有瀏覽器請求都必須遵循 HTTPS。 例如，設定 Cookie 時，您需依下列方式配對屬性的值：`Set-Cookie: example_session=test12; SameSite=None; Secure`。 如果未賦予正確標籤，Cookie 在較新版的瀏覽器中將無法使用，並遭到拒絕。

* `Lax`：允許僅針對採用&#x200B;*安全* (唯讀，例如 `GET`) HTTP 方法的最上層導覽傳送跨網站要求，連同相同網站的 Cookie。

* `Strict`：不會針對任何第三方網站請求傳送同網站 Cookie。 只有當 Cookie 的網站與 URL 列中的網站相符時，才會傳送 Cookie。
...
如果您的網站使用訪客ID服務，該服務會將第三方HTTP呼叫重新導向其HTTPS端點，這可能會增加延遲，但表示您不需要變更您的設定。

#### 當您針對多個網域使用一個 CNAME 時，請變更 SameSite 值 {#samesite-one-cname}

>[!NOTE]
>
>下列資訊僅與未使用訪客ID服務的網站有關。
...
>[!MORELIKETHIS]
>
>[可減輕瀏覽器Cookie限制之影響的選項Apple 的新 App Tracking Transparency Framework 對 Adobe Analytics 的影響](https://experienceleaguecommunities.adobe.com/t5/adobe-analytics-discussions/the-impact-of-apple-s-new-app-tracking-transparency-framework-on/td-p/401833)
