...
**我習慣了許多報表中的區段下拉式清單。 如何在 [!UICONTROL Analysis Workspace] 中加以重新建立？**

下拉式篩選器是 [!UICONTROL Analysis Workspace] 可啟用分段下拉式清單。 在工作區專案中：

1. 在您要納入下拉式篩選器的元件上，按住Ctrl鍵並按一下(Windows)，或按住Cmd鍵並按一下(Mac)。 不僅限於區段；下拉式篩選器中可包含任何元件。
2. 將元件群組拖曳至標示為「將區段放置在此處」的工作區區域中。請按住 Shift 再放開。

存取此 [!UICONTROL 工作區] 專案現在可以使用此下拉式篩選器，將區段或其他元件套用至專案。 如需詳細資訊，請參閱 Adobe Analytics 工具指南中的[面板概觀](/help/analyze/analysis-workspace/c-panels/panels.md)。
