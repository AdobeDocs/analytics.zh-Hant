---
description: Adobe Analytics中的各種報表可顯示「未指定」、「無」、「其他」或「未知」，視檢視的特定報表而定。 一般來說，此行項目表示變數未定義或是不可用。
title: 報告中的「未指定」、「無」、「其他」和「未知」
...
* **只引發一個變數的劃分報表：**&#x200B;將劃分套用至變數時，必須考慮該變數的每一個例項。如果第二個變數未顯示，或是從先前的點擊持續存在，則維度項目為「未指定」。
