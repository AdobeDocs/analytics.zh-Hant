...
| 報告套裝 | 說明 |
|---|---|
| ![重新發佈套裝](/help/assets/filter-reportsuite.png){width="300"} | **[!UICONTROL 報表套裝]**&#x200B;區段可讓您篩選報表套裝。 <ul><li>您可以![搜尋](/help/assets/icons/Search.svg) *搜尋報表套裝*&#x200B;以搜尋可用來篩選的報表套裝。</li><li>您可以選取多個報表套裝。 可用的報表套裝取決於篩選器面板中其他區段所做的選擇。</li><li>這些數字是表示：<ul><li>**(2)**：選取的報表套裝數目（若選取一或多個報表套裝）。</li><li>**⃣**：目前篩選產生的專案可用的報表套裝數目。</li><li>4︎：與特定報表套裝相關聯的專案數。</li></ul></li></ul> |
...
>[!WARNING]
>
>分類規則產生器將於&#x200B;**2027年2月28日**&#x200B;棄用。 一旦可用，請切換成使用[分類集規則](/help/components/classifications/sets/manage/rules.md)體驗，以確保繼續正常運作。
>
