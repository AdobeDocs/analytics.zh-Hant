...
## Reports & Analytics 生命週期結束公告 {#ra-eol}

>[!IMPORTANT]
>
>Adobe 已停止支援 Reports &amp; Analytics 及其隨附的報告和功能，自 **2024 年 1 月 17 日**&#x200B;起生效。 當時，Reports &amp; Analytics 及其所有報告和排程都已停止運作。 支援 Reports &amp; Analytics 的報告、視覺效果和基礎技術不再符合 Adobe 的技術標準。 大部分的 Reports &amp; Analytics 功能都可在 Analysis Workspace 中使用。 如需詳細資訊，請參閱[使用範本](/help/analyze/analysis-workspace/templates/use-templates.md)。
> 
...
>[!IMPORTANT]
>
>此外掛程式由 Adobe Consulting 提供，協助您從 Adobe Analytics 中獲得更多價值。 Adobe 客戶服務不提供此外掛程式的支援，包括安裝或疑難排解在內。 如果您需要與此外掛程式有關的協助，請聯絡貴組織的 Adobe 帳戶團隊。 他們可幫您安排顧問會議，以尋求協助。
...
>[!AVAILABILITY]
>
>本區段所述功能僅適用於已擁有該功能授權的現有客戶。 此功能不再作為額外附加元件提供給現有或新客戶。
>
...
當量度的回顧期間內出現多個值時，歸因模型會決定哪些維度項目可獲得該量度的貢獻度。 只有在回顧期間內設定多個維度項目時，歸因模型才適用。 如果只設定一個維度項目，則無論使用何種歸因模型，該維度項目都會獲得 100% 的貢獻。
...
| ![首次接觸](/help/assets/icons/AttributeFirstTouch.svg) | 首次接觸 | 將 100% 的貢獻給予在歸因回顧期間內首次出現的接觸點。 組織通常會使用此模型了解品牌認知度或客戶贏取。 |
| ![線性](/help/assets/icons/AttributeLinear.svg) | 線性 | 每個在轉換前出現的接觸點都會獲得相等的貢獻度。 那在轉換週期較長或需要較頻繁的客戶參與度時會很有用。 組織通常會使用此歸因模型來衡量行動應用程式通知的有效性，或用於訂閱型產品。 |
...
| ![同一次接觸](/help/assets/icons/AttributeSameTouch.svg) | 同一次接觸 | 會將 100% 的貢獻度分配給轉換發生時的相同事件。 如果接觸點不是在與轉換相同的事件中發生，就會歸類於「無」之下。 這種歸因模型有時會被視為根本沒有歸因模型。 那在不希望其他事件的值影響量度如何給予維度項目評分時，就很有價值。 產品或設計團隊可以使用此模型來評估轉換發生的頁面成效。 |
| ![U 型](/help/assets/icons/AttributeUShaped.svg) | U 型 | 會將 40% 的貢獻度分配給第一次互動，再將 40% 的貢獻度分配給最後一次互動，剩下的 20% 則分配給兩者之間的任何接觸點。 對於只有單一接觸點的轉換，會分配 100% 的貢獻度。 如果是具有兩個接觸點的轉換，兩者會平均獲得 50% 的貢獻度。 這種歸因模型最適合用於最重視第一次和最後一次互動，但又不想完全忽略在之間其他互動的情況。 |
| ![J 曲線](/help/assets/icons/AttributeJCurve.svg) | J 曲線 | 會將 60% 的貢獻度給予最後一次互動，再將 20% 的貢獻度給予第一次互動，剩下的 20% 則分給兩者之間的任何接觸點。 只有單一接觸點的轉換則會獲得 100% 的貢獻度。 如果是具有兩個接觸點的轉換，最後一次互動會獲得 75% 的貢獻度，第一次則獲得 25% 貢獻度。 與 U 型類似，此歸因模型偏好第一次和最後一次互動，但更加側重最後一次互動。 |
| ![反向 J](/help/assets/icons/AttributeInverseJ.svg) | 反向 J | 會將 60% 的貢獻度給予第一個接觸點，再將 20% 的貢獻度給予最後一個接觸點，剩下的 20% 則分給兩者之間的任何接觸點。 只有單一接觸點的轉換則會獲得 100% 的貢獻度。 如果是具有兩個接觸點的轉換，第一次互動會獲得 75% 的貢獻度，最後一次互動則獲得 25% 貢獻度。 與 J 型類似，此歸因模型偏好第一次和最後一次互動，但更加側重第一次互動。 |
| ![時間耗損](/help/assets/icons/AttributeTimeDecay.svg) | 時間耗損 | 會以自訂的半衰期參數 (預設為 7 天) 進行指數耗損。 每個管道的權重須視接觸點啟動和最終轉換之間所經過的時間量而定。 用於判斷評分的公式為 `2^(-t/halflife)`，`t` 代表接觸點和轉換之間的時間量。 所有接觸點隨後都會標準化至 100%。 適用於需要衡量特定且重要事件歸因的情境。 在此事件之後發生轉換的時間愈久，獲得的貢獻度就愈少。 |
| ![自訂](/help/assets/icons/AttributeCustom.svg) | 自訂 | 可指定想給予第一個接觸點、最後一個接觸點以及兩者之間任何接觸點的權重。 如果您輸入的自訂數字加總並非 100，系統也會將指定值標準化為 100%。 只有單一接觸點的轉換則會獲得 100% 的貢獻度。 如果是具有兩個接觸點的互動，系統會忽略中間參數。 然後將第一和最後一個接觸點標準化為 100%，再按照設定分配貢獻度。 這個模型適合想要完整控制其歸因模型，且具有其他歸因模型無法滿足之特定需求的分析人員。 |
| ![演算法](/help/assets/icons/AttributeAlgorithmic.svg) | 演算法 | 使用統計技巧動態決定所選量度的最佳貢獻度配置。 用於歸因的演算法以合作賽局理論中的 Harsanyi 利益為基礎。 Harsanyi 利益是 Shapley 值解法 (命名自諾貝爾經濟學獎得主 Lloyd Shapley) 的泛論，即在一場對結果貢獻不均等的賽局中，在玩家之間分配評分。<br>從高層次來看，歸因是以玩家的聯盟計算，其中盈餘必須公平地分配。 每個聯盟的盈餘分配則是根據每個子聯盟先前產生的盈餘 (或先前參與的維度項目) 遞迴決定。 如需更多詳細資訊，請參閱John Harsanyi和Lloyd Shapley的原始檔案：<br>Shapley， Lloyd S. (1953). n 人賽局的值。 *遊戲理論的貢獻，2(28)*，307-317.<br>Harsanyi，John C。 (1963). n 人合作賽局的簡化議價模型。 *International Economic Review 4(2)*, 194-220。 |
...
1. 9 月 24 日當天，您的行銷團隊會寄送電子郵件給對方，郵件內含對方購物車中某些商品的抵用券。 對方使用了抵用券，但也造訪了數個其他網站，看看是否有其他抵用券可用。 對方透過多媒體廣告找到了其他優惠券，最終以 $50 美元的價格購買商品。
...
| **頁面的探索性序列 (臨時分析)** | 是</br>結合預先定義的探索性分析。 在使用路徑上預先定義的節點時，將使用最終路徑 (只要訪客最終從一個預先定義的節點移到另一個預先定義的節點，就會納入計算)。 也可以顯示立即 (非最終) 的下一個節點。 | 有限的</br>讓您按一下右鍵並在自由格式表格中檢視立即的流失。 | 是</br>僅探索性分析。 始終在節點之間的一個維度實例內。 這表示每個節點顯示的是路徑上立即 (非最終) 的下一個接觸點。 |
