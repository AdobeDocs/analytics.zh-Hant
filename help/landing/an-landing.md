...
您登入 Adobe Experience CLoud 和 Analytics 後，可在左下方角落處啟用「[!UICONTROL 全新登陸頁面 - Beta 版]」的切換按鈕。存取切換按鈕是每個組織的用戶特定按鈕，而非公司特定的按鈕。
...
* 上方 Analytics 功能表變更：「**[!UICONTROL 報表]**」上方功能表已不存在。多數報表現在都列於左邊欄內的「[!UICONTROL 報表]」功能表中。
* 左邊欄有三個標籤：[!UICONTROL 專案]、[!UICONTROL 報表]和[!UICONTROL 學習]。我們會在以下幾個部分中更詳細地討論這些內容。
...
| 顯示較少/較多 | 在不顯示和顯示橫幅之間切換：![](assets/top-banner.png) |
...
| 名稱 | 專案名稱。 |
| 釘選圖示 | **最新** 您現在可以釘選項目，讓這些項目&#x200B;*隨時*&#x200B;顯示在您清單的最前面。 |
...
這是新功能表及其子功能表。如果找不到特定報表，請執行「搜尋頁面」後尋找。
...
| 管理員是否可以為他們的用戶指定此全新登陸頁面？ | 否，管理員不可以代表他們的用戶指定此全新登陸頁面。個人用戶必須自己開啟切換功能。 |
