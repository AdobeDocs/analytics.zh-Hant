...
| 產品 | 說明 | 文件連結 |
|--- |--- |--- |
| Analysis Workspace | 用於建立強大、自訂的分析專案，並讓使用者都能自行獲得深入分析的瀏覽器解決方案。提供比 Report &amp; Analytics 更多的報表靈活性。 | [adobe.ly/aaworkspacedocs](https://docs.adobe.com/content/help/zh-Hant/analytics/analyze/analysis-workspace/analysis-workspace-features.html) |
| Report &amp; Analytics (前稱為 SiteCatalyst) | 用於報告和分析的瀏覽器解決方案。Analytics 套裝中的入門工具。 | [Reports &amp; Analytics 首頁](https://docs.adobe.com/content/help/zh-Hant/analytics/analyze/reports-analytics/getting-started.html) |
...
| Adobe Exchange Data Connector (前身為 Genesis) | 從第三方應用程式匯入追蹤資料至 Analytics，以便在同一中央位置呈現端對端的效能資訊。 | [Data Connector 首頁](https://marketing.adobe.com/developer/documentation/genesis/c-overview-how-it-works) |
| 動態標籤管理 (DTM) | 可讓您管理所有網站上的分析、目標及其他標籤，不受網域數目影響。 | [DTM 首頁](https://docs.adobe.com/content/help/zh-Hant/analytics/implementation/implement-analytics-with-dtm/dtm-implementation-overview.html) |
...
| 量度 | 實施事件和計算量度的集合。 | [量度](https://docs.adobe.com/content/help/zh-Hant/analytics/analyze/analysis-workspace/components/apply-create-metrics.html) |
| 計算量度 | 能夠透過您的實施項目所擷取的現有量度中衍生出自訂量度。 | [計算量度](https://docs.adobe.com/content/help/zh-Hant/analytics/components/calculated-metrics/cm-overview.html) |
...
| Adobe Exchange Data Connector | 請參閱 [Analytics 工具](/help/landing/an-key-concepts.md) |  |
...
| 除錯工具選項 | 具備數個除錯程式和封包 Sniffer 可協助驗證您的實施情形，其中包括 Adobe Experience Cloud Debugger。 | [Adobe Debugger](https://chrome.google.com/webstore/detail/adobe-experience-cloud-de/ocdmogmohccmeicdhlhhgepeaijenapj?hl=tw) |
| 資料插入 API | 資料插入 API 提供的機制，可用於伺服器端資料收集與將資料提交至 Experience Cloud 伺服器。伺服器端資料收集僅會根據網站瀏覽器的要求和網站伺服器的回應收集資料，不會使用每個網頁上的 JavaScript 信標來將訪客資料傳輸至 Experience Cloud 伺服器。 | [透過 POST 實施 Adobe Analytics 資料插入 API 的步驟](https://helpx.adobe.com/tw/analytics/kb/data-insertion-api-post-method-adobe-analytics.html) |
