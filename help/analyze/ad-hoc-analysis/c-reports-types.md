...
seo-description: 說明 Experience Cloud 中使用的報表類型。
seo-title: 報表類型
solution: Analytics
title: 報表類型
...
1. To view a full report from the cell, click **[!UICONTROL Launch Trend Report]**.
   ...
   Alternatively, right-click the cell, then click **[!UICONTROL Trend Cell]**. 您也可以選取多個儲存格，然後執行此項工作。
...
轉換漏斗報表可以顯示特定度量事件之間的轉換百分比。您可使用此報表來瞭解產生銷售額的點進次數，以及售出件數。例如，[!UICONTROL 「產品轉換」]報表可以顯示「購物車」事件相較於「瀏覽」事件的百分比，再根據這些事件顯示「訂購」、「收入」和「件數」總計。
