...
Report Builder 需要驗證身分，才能從您的報表套裝中建立資料請求。視您在中進行的設定而定，登入Report Builder有時會發生問題 [!DNL Analytics] 或您的網路。

* **無效的登入公司**：此錯誤最常發生在登入公司未正確輸入或發生網路活動問題時。 請執行下列動作：
...
* **儲存格傳回0 （沒有值）**：Excel工作表名稱中出現撇號或單引號，會導致Report Builder未傳回任何值。 (此為 Microsoft Excel 的限制)。
...
* **此功能只適用於已開啟的活頁簿。**：如果未在Excel中開啟任何活頁簿（試算表檔案），且您按一下Report Builder工具列中的任一圖示時，此訊息便會出現。 此外，工具列將成為停用狀態，直到您開啟試算表為止。然而，在工具列仍為啟用狀態時，您可以按一下線上說明圖示而不致發生此錯誤。
