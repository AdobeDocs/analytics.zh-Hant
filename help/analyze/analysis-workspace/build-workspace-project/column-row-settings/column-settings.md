...
| 使用百分比限制 | 將限制範圍變更為以百分比而非絕對值為基礎。 此設定適用於僅有百分比的量度 (例如反彈率)，以及含有計數和百分比的量度 (例如頁面檢視)。 |
| 自動產生 | 根據資料自動計算上限／中限／下限。 上限是此欄中的最大值。下限是最小值，而中點是上限和下限的平均。 |
| 自訂 | 手動指派上／中/下限。 這可提供由您決定欄的值是好、中等或壞的彈性。 |
