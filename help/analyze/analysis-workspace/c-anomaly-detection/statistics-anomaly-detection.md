...
這些假日是根據許多客戶資料點的廣泛統計分析所選出，用以識別哪些假日對最多客戶的趨勢影響最大。 此清單當然不是對所有客戶或業務週期都詳盡無遺，但套用這些假日可大幅改善幾乎所有客戶資料集的整體演演算法效能。
...
每週和每月趨勢不會呈現每日或每小時顆粒度中所見的相同每週或每日趨勢，因此會使用不同的演算法。 對於每週和每月，兩步驟的極端值偵測方法稱為廣義極端學生化偏差(GESD)測試。 此測試會考慮結合調整盒形圖方法（發現離群值的非引數方法）的預期異常最大數量，以決定離群值最大數量。 兩步驟為：

1. 調整盒形圖函數：此函數會根據輸入資料決定異常值的最大數量。
...
1. 系統會針對每個維度計算 Cramer&#39;s V 檢定統計值。 在以下範例中，假設有一個列聯表，內含兩個期間內，各個國家/地區的頁面檢視次數。
...
1. 對於每個維度項目，使用皮爾遜殘差 (PR) 來測量異常量度和每個維度項目之間的關聯性。 PR遵循標準常態分佈，此分佈可讓演演算法比較兩個隨機變數的PR，即使偏差不可比較。 實際上，這個錯誤是未知的，並且是使用有限樣本校正來估計的。
   ...
   計算最終結果時，每個維度項目的得分會經過 Cramer&#39;s V 評估值的加權，並重新調整為介於 0 和 1 之間的數字，以便提供貢獻分數。
