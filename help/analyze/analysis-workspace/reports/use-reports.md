...
Analysis Workspace 中的預先建立報告可讓您快速了解最常見的報告情境。以下範例是您可以使用預先建立報告回答的問題：
...
以下資訊說明如何從 Analysis Workspace 中的「[!UICONTROL 報告]」標籤存取和使用預先建立的報告。
...
1. 在搜尋欄位中，開始輸入要尋找報告的名稱，然後從報告清單中選取它。您也可以依據 prop、eVar 和事件編號來搜尋報告清單。<!-- still true? -->

   或

   選取要查看的報告類別，然後從報告清單中選取報告。

   >[!TIP]
   >
   >   若要使用方向鍵瀏覽選單，請按正斜線鍵 (/)，然後按向下鍵。按 Enter 鍵載入所選報告。

有關可用報告的清單，請參閱下方的[可用報告](#available-reports)區段。
...
1. (選用) 選取「[!UICONTROL **專案**]」>「[!UICONTROL **儲存**]」，將報告將另存為專案。
   ...
   該報告會另存為新專案，而不會修改現有的報告。有關將報告另存為專案的詳細資訊，請參閱「[儲存專案](/help/analyze/analysis-workspace/build-workspace-project/save-projects.md)」。

## 可用報告

下表包含可用預先建立報告的完整清單。

其中包括一些最熱門的預先建立報告 (流量、流失、行銷管道和即時報告)。
