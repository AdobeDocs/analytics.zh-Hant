...
策劃功能可讓您在共用專案前先限制元件 (維度、量度、區段、日期範圍)。 收件者在開啟專案時，將會看到您為他們組織好的數量有限的元件。 策劃是可選作業，但建議您在共用專案前先執行此步驟。
...
注意：非管理員檢視角色無法存取專案中的左側邊欄，因此已從下表中省略。

| 組織型別 | 管理員 | 非管理員專案擁有者或編輯角色 | 非管理員重複角色 |
|---|---|---|---|
| **已組織的虛擬報表套裝** | 所有未經策劃的虛擬報告套裝元件 | 此角色擁有或已與其共用之非監管的虛擬報告套裝元件 | 此角色擁有或已與其共用之非監管的虛擬報告套裝元件 |
