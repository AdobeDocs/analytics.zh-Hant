...
如果您使用工作區，您可以建立相對於面板日曆的日期範圍元件，以便在左側邊欄（或在元件內）預覽的資料以面板日期範圍為基礎。 三種常見的使用案例包括組合圖、關鍵量度摘要，以及自由表格日期範圍。
...
1. 選擇或取消選擇 **建立相對於面板日曆的日期範圍元件**.
   * 選取選項，使日期範圍元件相對於面板日曆。
   * 取消選取此選項表示如果面板日期範圍已變更，面板中的日期範圍（關鍵量度摘要、下拉式圖表或紫色日期片）將不會更新。 這是預設值。

   ![相對面板日期](assets/relative-date-snippet.png)
