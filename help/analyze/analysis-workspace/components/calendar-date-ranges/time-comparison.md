...
1. 對表格列按一下滑鼠右鍵並選取「**[!UICONTROL Compare Time Periods]**」。
...
1. 您有以下這些比較選項，取決於您如何設定表格的日期範圍：
   ...
   >[!NOTE]
   >
   >如果選取自訂天數，例如 10 月 7 日到 10 月 20 日 (14 天的範圍)，您只有兩個選項可選：「**[!UICONTROL Prior 14 days before this date range]**」和「**[!UICONTROL Select range]**」。
...
1. (選用) 如同任何其他 Workspace 專案，您可以根據這些時間比較建立視覺效果。例如，這張長條圖：
   ...
   請注意，若要在長條圖中顯示變化百分比，您必須勾選「[!UICONTROL Visualization Settings]」中的「[!UICONTROL Percentages]」設定。

## 新增時段欄以進行比較 {#section_93CC2B4F48504125BEC104046A32EB93}

您現在可以在表格的每個欄中新增時段，以新增不同於行事曆設定的時段。這是另一種比較日期的方式。

1. 對表格中的欄按一下滑鼠右鍵並選取「**[!UICONTROL Add Time Period Column]**」![](assets/add-time-period-column.png)
...
適用於所有表格的新設定可讓您用來「**[!UICONTROL Align Dates from each column to all start on the same row (applies to entire table)]**」。「套用至整個表格」的意思是，舉例來說，如果您劃分表格，並且變更劃分的某項設定，則會變更整個表格的該項設定。
