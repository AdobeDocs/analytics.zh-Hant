...
您可以將任何元件類型（維度、維度項目、事件、量度、區段、區段範本、日期範圍）拖放至面板頂端的區段拖放區，以建立即時區段。
...
* 對於完整維度和事件，分析工作區會建立「存在」點擊區段。 例子：「點撃 eVar1 存在的位置」或「點撃 event1 存在的位置」。
