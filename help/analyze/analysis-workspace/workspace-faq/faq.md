...
| 問題 | 回答 |
|--- |--- |
| 使用 Analysis Workspace 的必要條件為何？ | [透過 Adobe Experience Platform Launch 將資料傳送至 Adobe Analytics](/help/implement/launch/validate-publish-prod.md)：使用 Analysis Workspace 需要先有效實作工具。請確認您的組織確實將資料傳送至 Adobe，再開始使用工具。其他實作方式 (例如舊版手動實作) 也可有效運用。 |
...
| Analysis Workspace 專案中可以顯示多少報告套裝？ | 現在，您可以在 Analysis Workspace 中以更豐富的[多個報告套裝](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/build-workspace-project/multiple-report-suites.html)資料建立專案。 |
| 如何實作 Analysis Workspace？ | 不需要特別實作。所有已安裝 Analytics Standard 或 Premium 的公司都能使用 Analysis Workspace。但是將套用內容的標準權限 (例如報告套裝和專案元件)，以及適用於組織和共用專案。請參閱[管理和存取需求](/help/analyze/analysis-workspace/workspace-faq/frequently-asked-questions-analysis-workspace.md)。 |
