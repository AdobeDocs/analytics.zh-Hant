...
| 因素 | 定義 | 最佳化 |
| --- | --- | --- |
| 查詢數 | 向 Adobe 提出以擷取專案中顯示之資料的查詢 (請求) 總數。查詢包括表格的排名請求、異常偵測、走勢圖、左側邊欄中顯示的元件等。不包括摺疊的面板和視覺效果。建議為 100 個。 | 將資料分割為多個專案，以符合特定目的或供相關人士使用，盡可能簡化專案。使用標籤將專案組織成主題，並使用[直接連結](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/curate-share/shareable-links.html)，建立內部目錄，讓相關人士可更輕鬆找到所需項目。 |
...
| 併發查詢 | 貴組織同時請求Adobe的查詢數。 每個組織至少有5個並行查詢。 | 如果報表需要很長的時間，通常是因為它位於與其他報表一起排列的佇列中。 這表示您的組織正嘗試針對特定報表套裝執行許多同時請求。 查詢可來自API請求、報表UI(Analysis Workspace、Reports &amp; Analytics、Report Builder等)、已排程專案、已排程報表、已排程警報，以及同時提出報表請求的使用者。 | 在一天中更平均分佈報表套裝的請求和排程。此外，盡可能將您的請求移至非高峰時間。 週一早上、週二早上和每月的第一個是尖峰報告時間。 |
| 資料中心容量 | 您和其他客戶在Adobe資料中心內共用的報告容量量。 | 這受您的組織和資料中心內其他組織同時提出的查詢數影響。 | 貴組織有權獲得一定容量，如果系統處於輕負載狀態，Adobe將向您轉移更多容量，超出您有權獲得的容量。 |
