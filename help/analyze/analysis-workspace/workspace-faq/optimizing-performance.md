...
| 併發查詢 | 貴組織同時向Adobe要求的查詢數。 | 您的組織針對特定報表套裝同時執行過多請求。查詢可來自API請求、報告UI（分析工作區、報告與分析、報告建立工具等）、計畫專案、計畫報表、計畫警報，以及提出報告請求的並行使用者。 | 在一天中更平均分佈報表套裝的請求和排程。此外，請盡可能將要求移至非尖峰時間。 週一早晨、週二早晨和每月的第一天都是高峰報告時間。 |
| 資料中心容量 | 您和其他客戶在Adobe資料中心內共用的報告容量。 | 您的組織和資料中心內的其他組織進行的併發查詢數。 | 如果空間允許，Adobe會最佳化。 貴組織有權取得固定容量，如果系統負載較輕，Adobe會將更多容量轉移給您，超過您的授權額度。 |
