...
| 因素 | 定義 | 最佳化 |
| --- | --- | --- |
| 查詢數 | 向 Adobe 提出以擷取專案中顯示之資料的查詢 (請求) 總數。查詢包括表格的排名請求、異常偵測、走勢圖、左側邊欄中顯示的元件等。不包括摺疊的面板和視覺效果。建議為 100 個。 | 將資料分割為多個專案，以符合特定目的或供相關人士使用，盡可能簡化專案。使用標籤將專案組織成主題，並使用[直接連結](https://docs.adobe.com/content/help/zh-Hant/analytics/analyze/analysis-workspace/curate-share/shareable-links.html)，建立內部目錄，讓相關人士可更輕鬆找到所需項目。 |
...
| 視覺效果複雜度 (區段、量度、篩選條件) | 專案本身新增的視覺效果類型 (例如，流失率與自由表格對比) 對專案效能的影響不大。視覺效果的複雜度會增加處理時間。 | 增加視覺效果複雜度的因素包括：<ul><li>要求的資料範圍</li><li>套用的區段數；例如，用來作為自由表格列的區段</li><li>使用複雜區段</li><li>[自由表格的靜態項目列或欄](https://docs.adobe.com/content/help/zh-Hant/analytics/analyze/analysis-workspace/visualizations/freeform-table/column-row-settings/manual-vs-dynamic-rows.html)</li><li>自由表格列所套用的篩選器</li><li>包含的量度數目，尤其是使用了區段的計算量度</li></ul> | 如果您注意到您的專案載入速度不如預期，可以的話，試著將一些區段取代為 eVar 和篩選器。<br><br>如果您發現自己持續使用公司重要資料點的區段和計算量度，請考慮改良實作，用更直接的方式擷取這些資料點。使用 Adobe Experience Platform Launch 之類的標記管理器和 Adobe 的處理規則，可以快速輕鬆地執行實作變更。 |
| 報表套裝的大小 | 收集到報表套裝中的資料量。 | - | 洽詢您的實作團隊或 Adobe 專家，判斷是否有可行的實作改善項目能改善 Adobe Analytics 的整體使用體驗。 |
| 併發查詢 | 您的組織同時要求Adobe的查詢數。 每個組織至少有5個並行查詢。 | 如果報表需要很長時間，通常是因為它和其他報表在佇列中。 這表示您的組織正嘗試針對特定報表套裝執行許多並行請求。 查詢可能來自API請求、報告UI(Analysis Workspace、報告與分析、Report Builder等)、計畫專案、計畫報表、計畫警報以及提出報告請求的並行使用者。 | 在一天中更平均分佈報表套裝的請求和排程。此外，請盡可能將要求移至非尖峰時間。 週一早晨、週二早晨和每月的第一天都是高峰報告時間。 |
| 資料中心容量 | 您和其他客戶在Adobe資料中心中共用的報告容量。 | 這受您的組織和資料中心內其他組織進行的並行查詢數的影響。 | 您的組織有權獲得固定容量，如果系統處於輕負荷狀態，Adobe將向您轉移更多容量，超過您有權獲得的額外容量。 |
