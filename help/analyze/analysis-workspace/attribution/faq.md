...
我們初次推出行銷管道時，只提供「首次接觸」和「上次接觸」維度。有了現行版本的歸因，就不再需要明確的首次/最後接觸維度。Adobe提供一般 [!UICONTROL 行銷管道] 和 [!UICONTROL 行銷管道詳細資料] 維度，以便將維度與所需的歸因模型搭配使用。 這些通用維的行為與相同 [!UICONTROL 上次接觸管道] 維度，但標籤不同，這是為了避免搭配不同歸因模型使用行銷管道時產生混淆。
