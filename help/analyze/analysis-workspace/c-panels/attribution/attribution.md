...
>[!IMPORTANT] Adobe Analytics Ultimate、Prime、Select和Foundation SKU的所有客戶都可使用歸因。

[歸因IQ](../../attribution-iq.md) 是一組功能，可讓您將許多新類型的歸因模型新增至自由表格、視覺化和計算量度。 所有的歸因模型都有兩個元件：
...
| ![演算法](assets/algorithmic.png) | [演算法](/help/analyze/analysis-workspace/c-panels/attribution/algorithmic.md) | (注意： 演算法歸因目前進行的測試有限。) 使用統計技巧動態決定所選量度的最佳評分配置。 | 為貴公司選擇正確的歸因模型時，有助於避免猜測或試探。 |
...
* **自訂回顧視窗：** (注意： 自訂回顧視窗目前處於有限測試中。[詳細資訊](https://docs.adobe.com/content/help/en/analytics/landing/an-releases.html))可讓您將歸因視窗從報告日期範圍延長至最多90天。 自訂回顧視窗會評估在報告時段內的每個轉換。 例如，若是在2月20日發生的轉換，10天的回顧視窗會在歸因模型中評估從2月10日到20日的所有維度觸點。
...
>[!NOTE] 如果區段隱藏包含量度的點擊，這些量度例項不會歸因於任何維度。不過，類似的報表篩選器只會隱藏某些維度值，而不會對根據歸因模型處理的量度造成任何影響。因此，具有可比較定義的區段和篩選器，有時候會傳回較小的區段值。
