...
「Analytics for Target」(A4T)面板可讓您分析分析分析工作區中的Adobe Target活動和體驗。 此外，您還可以檢視最多3個成功度量的提升度與信賴度。 若要存取A4T面板，請按一下最左側的面板圖示，然後將「Analytics for Target」面板拖曳至您的分析工作區專案。
...
| 標準化量度 | 從「獨特訪客」、「瀏覽」或「活動印象」中選擇。 |
| 成功度量 | 從下拉式清單中最多選取3個標準成功事件，或從左側導軌拖放度量。 每個量度在轉譯面板中都會有專屬的表格和視覺化。 |
| 日曆 | 這會根據Adobe Target的活動日期範圍自動填入。 如有需要，您可加以變更。 |
...
| 可信度 | 學生t-test會計算信賴等級，指出如果再次執行測試，結果重複的可能性。<br>**注意：**信賴度是Adobe Target體驗的「鎖定度量」。 它無法劃分或與其他維度搭配使用。 |

與分析工作區中的任何面板一樣，您可以新增其他表格和視覺化來繼續分析，以協助您分析Adobe Target活動。

如需Analytics for Target報表的更多選項，請造訪A4T報表（連結即將推出）。