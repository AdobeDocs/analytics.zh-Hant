...
視覺效果圖例可協助您將來源表格內的日期與視覺效果中的繪製序列產生關聯。圖例為互動式 - 您可以選取圖例項目來顯示/隱藏視覺效果中的系列，如果您想簡化視覺化資料，這相當實用。
...
| **[!UICONTROL 詳細程度]** | 變更趨勢視覺效果的時間詳細程度。此變更也適用於資料來源表。 |
...
| **[!UICONTROL 顯示異常狀況]** | 透過顯示異常偵測來強化折線圖和自由格式表格。折線圖視覺效果的異常偵測包括一個預期值 (虛線) 和一個預期範圍 (陰影帶)。 |
...
您可以自訂您所建立的所有視覺效果設定。請參閱[使用者偏好設定](/help/analyze/analysis-workspace/user-preferences.md)，以瞭解更多詳細資訊。
...
| 選項 | 說明 |
| --- | --- |
| **[!UICONTROL 插入複製視覺效果]** | 將複製的視覺效果貼上 (插入) 至專案內另一個位置或完全不同的專案中。 |
