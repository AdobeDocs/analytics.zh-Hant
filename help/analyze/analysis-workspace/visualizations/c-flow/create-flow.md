...
1. 在專案中新增空白面板，然後按一下左側欄中的視覺效果圖示。

   或

   透過下列位置的「將視覺效果新增至面板」一節中所述的任何方式新增視覺效果： [視覺效果概述](/help/analyze/analysis-workspace/visualizations/freeform-analysis-visualizations.md).
...
流量組態的摘要會顯示在圖表的頂端。圖表中的路徑厚度與其活動成正比，活動較多的路徑看起來比活動較少的路徑厚。

若要深入研究資料，您有幾個選項：

* 流量圖是互動式的。將滑鼠移到圖表上方，可變更顯示的詳細資料。

* 當您在圖表中的節點上選取時，會顯示該節點的詳細資料。 再次在節點上選取以摺疊節點。
...
* 選取組態摘要旁的鉛筆圖示，以進一步編輯流量或使用不同選項加以重建。

* 您也可以前往&#x200B;**[!UICONTROL 「專案]** > **[!UICONTROL 下載 CSV」]**，匯出並進一步分析您專案 .CSV 檔案的「流量」圖表。

## 篩選

將滑鼠游標停留在欄上方時，會出現一個篩選器。選取篩選器後，畫面就會顯示與現在自由表格中相同的篩選器對話方塊。 此篩選器的運作方式，與自由表格中的運作方式相同。
