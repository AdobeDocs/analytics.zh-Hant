...
如果您偏好先將數個元件新增至表格，然後再轉譯資料，則可啟用「自由表格產生器」。啟用產生器後，許多維度、劃分、量度和區段都可直接拖放使用，方便建立表格以供回答更複雜的問題。Data will not update on-the-fly, it will update once you click **[!UICONTROL Build]**.
...
* **[!UICONTROL Breakdown by Position]** 對於表的每個級別(在傳統的自由表格中，預設值為 **[!UICONTROL Breakdown by Item]**)。
...
* 按一下右鍵表標題並選擇 **[!UICONTROL Copy to Clipboard]**。 這能匯出 (可檢視的) 完整表格。
