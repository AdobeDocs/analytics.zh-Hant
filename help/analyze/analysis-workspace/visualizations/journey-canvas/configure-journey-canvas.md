...
1. 設定下列任一顯示於視覺化圖表上方的設定：
   ...
   | [!UICONTROL **與**]&#x200B;比較 | 用來將目前歷程資料與前一期資料相比的日期範圍。 您可以選擇以下任何日期範圍來比較：<ul><li>**[!UICONTROL 4週前]**</li><li>**[!UICONTROL 2季之前]**</li><li>**[!UICONTROL 前1年]**</li><li>**[!UICONTROL 自訂日期範圍]**</li></ul><p>當您選取比較日期範圍時，歷程中的每個節點都會根據主要量度，顯示目前日期範圍和所選比較日期範圍之間的百分比變化。 這可讓您識別歷程的執行效能是否優於先前時段。</p> |
...
1. 從左側邊欄將量度、維度、維度項目、區段或日期範圍拖曳至畫布上。 但不支援計算量度。
   ...
   | 量度 | 2 個現有節點之間的箭頭 | 節點會顯示在元件放置的兩個現有節點之間，且連接至兩個現有節點。 (如需更多資訊請參閱[連接節點](#connect-nodes)。) |
   ...
   | 維度 | 連接 2 個現有節點的箭頭 | 系統會針對 (最終到達第二個節點的人員或工作階段的) 第一個節點之後的第一個事件中，前 3 大的維度項目建立 3 個節點。 節點會顯示在放置元件處的兩個現有節點之間，且各節點都會連接至這兩個現有節點。 (**注意：**&#x200B;如果只顯示 1 或 2 個節點，表示只有 1 或 2 個維度項目具有可用資料。 如果沒有顯示任何節點，表示任何維度項目皆無可用的資料。 於此情況下，請嘗試將其新增至歷程的不同點、調整視覺化圖表的日期範圍，或選擇不同的維度。)<p>將維度拖放到畫布上時按住 Shift 鍵，即可將其新增為包含 3 個維度項目的單一節點。 (如需更多資訊請參閱[連接節點](#connect-nodes)。)</p> |
   ...
   | 多個元件 | 連接 2 個現有節點的箭頭 | **如果沒有任何元件是維度：**<p>各元件會在元件放置處顯示為獨立節點，且每個節點皆與兩個現有節點連接。 (如需更多資訊請參閱[連接節點](#connect-nodes)。)<p>將元件放到畫布上時按住 Shift 鍵，即可將它們新增為一個合併節點。 （元件必須屬於相同型別，才能合併成單一節點。） (如需更多資訊請參閱[合併節點](#combine-nodes)。)</p><p>**如果您要新增的任何元件是維度：**</p><p>各元件會在元件放置處顯示為獨立節點，且每個節點皆與兩個現有節點連接。</p><p>一次只能新增一個維度。 新增維度時，系統會針對 (最終到達第二個節點的人員或工作階段的) 第一個節點之後的第一個事件中，前 3 大的維度項目建立 3 個節點。 各節點都連接到兩個現有節點。 (如需更多資訊請參閱[連接節點](#connect-nodes)。)</p><p>按住 Shift 鍵，將元件放到畫布上，將其新增為合併節點。 前 3 大維度項目會與各節點結合，而各節點皆連接至兩個現有節點。 (如需更多資訊請參閱[合併節點](#combine-nodes)。)</p> |
...
1. 繼續依照下列各節所述自訂歷程。 您可以連線節點、重新命名節點、套用劃分、新增時間限制等。

### 根據現有節點顯示熱門的節點

您可以根據畫布上已存在的節點，自動顯示排名最前的立即節點或最終節點。 您可以將熱門節點新增至歷程畫布，或在自由格式表格中檢視。
...
您可以選取節點，並在歷程中顯示緊接在節點後面的前幾個立即或最終維度專案。 您可以將前 3 大維度項目新增至歷程畫布做為獨立節點，或在自由格式表格中檢視所有熱門的維度項目。
...
1. 選取要顯示立即或最終在選取節點之後的維度專案：

   * [!UICONTROL **緊接在**]&#x200B;之後：顯示直接在所選節點之後排名最前的維度專案。 當您想知道在歷程中的指定步驟之後立即發生什麼時，請使用此專案。 例如，您可以檢視訪客在造訪指定產品頁面後立即造訪的頁面。

   * [!UICONTROL **最終在**]&#x200B;之後：顯示在歷程中任何時間點晚於所選節點的排名最前的維度專案，不一定是下一個步驟。 當選取的節點是您分析中的關鍵事件，且您想要檢視人員之後在任何時間點做什麼，請使用此選項。 例如，購買後，您可以看到使用者最終在歷程稍後任何時間點瀏覽的重點頁面。

1. 選取您要顯示維度項目的位置：

   * [!UICONTROL **在歷程畫布中**]：將歷程中此節點之後的前 3 大節點新增至畫布。 每個節點都會在畫布上做為個別分支連接到您選取的節點。
...
您可以選取節點，並顯示歷程中緊接在它之前的前幾個立即或最終維度專案。 您可以將前 3 大維度項目新增至歷程畫布做為獨立節點，或在自由格式表格中檢視所有熱門的維度項目。

1. 在您要顯示歷程中該節點之前的熱門維度項目的節點上按一下滑鼠右鍵。

   在歷程中此節點不能有任何連接進來的現有節點。

1. 選取「[!UICONTROL **顯示此節點之前的熱門節點**]」。

1. 選取要顯示立即或最終在所選節點之前的維度專案：

   * [!UICONTROL **緊接在**]&#x200B;之前：顯示直接在所選節點之前排名最前的維度專案。 當您想知道在歷程中的指定步驟之前發生什麼時，請使用此專案。 例如，您可以檢視訪客在造訪指定產品頁面前立即造訪的頁面。

   * [!UICONTROL **最終在**]&#x200B;之前：顯示在歷程中任何時間點，在選取的節點之前排名最前的維度專案，不一定是先前的步驟。 當選取的節點是您分析中的關鍵事件，且您想要檢視人們在任何時候為其做的事時，請使用此選項。 例如，在購買前，您可以檢視訪客在歷程中任何先前時間點瀏覽過的最上層頁面。

1. 選取您要顯示維度項目的位置：

   * [!UICONTROL **在歷程畫布中**]：將歷程中此節點之前的前 3 大節點新增至畫布。 每個節點都會連接到您選取的節點，並在畫布上形成個別的分支。
...
歷程畫布中的合併節點是使用者歷程中的單一點 (節點)，包含 2 個或更多透過邏輯連接在一起的元件。
...
* 在畫布上，選取您要合併的節點，用滑鼠右鍵按一下其中一個選取的節點，然後選取&#x200B;**合併節點**。 您可以合併的人數是否有限制？
...
您可以將合併節點中包含的元件分割成它們自己的個別節點：

1. 用滑鼠右鍵按一下您要分割的節點，然後選取&#x200B;[!UICONTROL **將元件分割成節點**]。
...
當Journey Optimizer歷程包含相同兩個節點之間的多個路徑時（例如，導致相同下一個步驟的不同條件），每個路徑都會顯示為個別的箭頭。

#### 連接節點時的邏輯

當您連接歷程畫布中的節點時，會使用 THEN 運算子來連接節點。 這也稱為[循序細分](/help/components/segmentation/segmentation-workflow/seg-sequential-build.md)。

節點會以「最終路徑」連接，這表示只要訪客最後從一個節點移動到另一個節點，就會納入計算，而不管 2 個節點之間發生什麼事件。 使用者沿著路徑移動的配給時間，由容器設定決定。<!-- It can also be controlled by [adding a time constraint](#add-a-time-constraint-between-nodes). -->
...
1. 在歷程畫布視覺化圖表中，在您想要重新命名的節點上按一下滑鼠右鍵。
...
1. 選取要用於劃分的維度。
   ...
   如果您選擇在自由格式表格中檢視劃分，熱門維度項目會顯示在新的自由格式表格中，緊接在歷程畫布視覺化圖表上方。
...
您可以變更歷程中節點之間的箭頭曲率和樣式。

#### 變更箭頭的曲率

若要變更「歷程」畫布中的箭頭曲率：

1. 在「歷程」畫布視覺效果中，拖曳箭頭至兩個節點。

若要將箭頭重設回其預設位置：

1. 用滑鼠右鍵按一下要重設的箭頭。

1. 選取&#x200B;[!UICONTROL **重設箭號位置**]。

   如果相同的兩個節點之間存在多個箭頭，這些節點之間的所有箭頭都會重設為預設位置。

#### 變更箭頭的樣式

若要變更「歷程」畫布中的箭頭樣式：

1. 在「歷程」畫布視覺效果中，以滑鼠右鍵按一下要變更樣式之兩個節點之間的箭頭。

1. 選取&#x200B;[!UICONTROL **變更箭頭樣式**]，然後選取下列其中一個樣式：
...
您可以在歷程中一次刪除一個或多個箭頭。 當您刪除 2 個節點之間的箭頭時，這些節點將不再連接。 如果箭頭是較長路徑的一部分，則該路徑會中斷。
