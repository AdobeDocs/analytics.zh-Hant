...
seo-title: 異常偵測中使用的統計技術
title: 異常偵測中使用的統計技術
uuid: b6ef6a2e-0836-4c9a-bb7 e-01910199bb92
...
視報表中使用的日期粒度而定，會使用種不同的統計技術-尤其是每小時、每日、每周/每月異常偵測。各種統計技術概述如下。

## Anomaly detection for daily granularity {#section_758ACA3C0A6B4D399563ECABFB8316FA}

對於每日粒度報表，演算法會考量數個重要因素，盡可能提供最準確的結果。首先，演算法決定要根據可用資料的類型來套用哪種模型類型，以在兩個類別中選擇-以時間序列為基礎的模型或離群偵測模型(稱為功能篩選)。
...
我們根據許多客戶資料點的廣泛統計分析，識別哪些假日對最大數量的客戶趨勢影響最為重要，而選定這些假日。此清單當然不是對所有客戶或商業週期都詳盡無遺，但我們發現套用這些假日可大幅改善幾乎所有客戶資料集的整體演算法效能。

選取好模型，也確定報告日期範圍內的假日後，演算法會以下列方式進行:

1. 建構異常參考期間-這包括報告日期範圍前35天，以及相符日期範圍一年前的相符日期範圍(在上一年的不同日曆日可能發生的任何適用假日)。
1. 根據最新資料，測試目前期間內的假日 (不包括前一年) 是否異常。
1. 如果目前日期範圍內的假日為異常，則根據前一年的假日，調整目前假日的期望值和信賴區間 (考慮前後 2 天)。目前假日的校正是根據下列項目的最低平均絕對百分差:

   1. 相加效應
   1. 相乘效應
   1. YoY 差異

請注意以下範例中大幅改善了聖誕節和新年的效能:
...
接著，假日和 YoY 季節性異常偵測步驟從今年資料減去去年資料，然後使用上述兩步驟程序再次對資料進行迭代運算，驗證異常符合季節。這兩個資料粒度都使用選定報告日期範圍的 15 期回溯包含 (15 個月或 15 週) 以及 1 年前的對應日期範圍用以訓練。
