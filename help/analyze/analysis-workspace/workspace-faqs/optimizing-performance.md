...
雖然無法避免某些複雜因素，但請考慮降低區段複雜性的機會。 一般而言，您對區段標準的特定程度越高，就越好。 例如：

* 在容器中，使用區段頂端的單一容器會比一系列巢狀容器更快。
...
請記住，日期範圍（紫色元件）會覆寫面板的日期範圍。 因此，如果您使用不同日期範圍作為欄 (例如，「上個月」、「上週」和「昨天」欄)，面板日期範圍不必跨越所有的欄日期範圍。它只能設為昨天，因為自由表格中使用的資料範圍會覆寫面板。 如需在 Analysis Workspace 中使用日期範圍的相關資訊，請看[這段影片](https://www.youtube.com/watch?v=ybmv6EBmhn0)。
...
減少專案中的視覺化數目。 「分析工作區」會針對您新增的每個視覺效果在幕後執行許多處理，因此會優先處理對報表消費者最重要的視覺效果，並視需要將支援視覺效果分為個別、更詳細的專案。

## 視覺效果的複雜度 (區段、量度、篩選器)

專案本身新增的視覺效果類型 (例如，流失率與自由表格對比) 對專案效能的影響不大。視覺化的複雜性會增加處理時間。 增加視覺效果複雜度的因素包括：
...
請勿在一個專案中加入所有所需項目，盡可能針對特定用途或利害關係人群組建立個別專案。使用標籤將專案組織成關鍵主題，並與利益相關者群組共用相關專案。

如果需要更多的專案組織，請記 [住，直接連結](https://www.youtube.com/watch?v=6IOEewflG2U) 至專案是一個選項。 建立內部的專案索引，讓股東更容易找到他們要找的。

如果一個工作區需要許多面板，請先收合面板，再儲存和共用。 載入專案時，分析工作區將只載入展開面板的內容。 收合的面板在使用者展開之前不會載入。 這種做法有兩個優點：

* 收合面板可節省專案的整體載入時間
* 收合面板是組織專案的絕佳方式，讓報表的使用者符合邏輯
