...
* 標準流量：根據指定的行項目來顯示度量。 例如，在「頁面」報表中，「造訪」量度會顯示該特定頁面的造訪次數。
* 標準商務：「收入」、「訂購」及「結帳」等度量。
* 總度量：根據報表日期範圍來顯示度量，而不考慮行項目。 此量度與其個別分析報表底部的總計相同。
* 參與率
* 計算

維度是非數值和日期，例如性別、月份、年齡、忠誠度、監視器解析度等。 維度是可檢視及可比較之度量資料的說明和特性，如劃分報表中的維度。
