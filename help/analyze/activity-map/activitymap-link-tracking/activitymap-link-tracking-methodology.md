...
字串，用於識別要用作的某些上階元素(parent、parent.parent、...)中的地區ID的標籤屬性 `s.linkObject`，即， **點按的元素**.
...
接收點按的函式 `HTMLElement` 且應傳回代表已點按連結的字串值。 如果傳回值為 false (null、未定義、空字串、0)，將不會追蹤任何連結。
...
函數，接受被點按的 HTMLElement，且應傳回代表&#x200B;**在點按時連結所在地區的字串值。**&#x200B;如果傳回值為 false (null、未定義、空字串、0)，將不會追蹤任何連結。
