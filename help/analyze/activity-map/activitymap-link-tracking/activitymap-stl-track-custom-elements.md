...
當直接從HTML元素的點按時事件呼叫s.tl()函式時，Activity map會在載入網頁時顯示該元素的覆蓋圖。 範例:
...
當s.tl()函式未直接從HTML元素的點按時事件呼叫時，Activity map只有在使用者點按該元素後，才能顯示覆蓋圖。 以下是間接呼叫 s.tl() 函數的範例:
...
Activity Map 覆蓋動態內容連結的最佳方式是具備自訂的 ActivityMap.link 函數設定，藉此呼叫其傳回值已傳遞至 s.tl. 的相同函數。以下是一個例子：
