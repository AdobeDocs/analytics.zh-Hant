...
| [!UICONTROL **報告套裝**] | 識別與專案相關聯的報告套裝。<br>面板內的表格和視覺效果是從面板右上角所選報告套裝衍生而得。 該報告套裝也會決定左側邊欄內會有哪些元件。 在專案中，您可以使用一或多個報告套裝，多寡視您的分析使用情形而定。 報告套裝清單是根據相關性排序。 Adobe 是根據目前使用者最近使用套裝情形和多常使用套裝，以及組織內多常使用套裝等來定義關聯性。 |
| [!UICONTROL **所有者**] | 識別建立專案的人員。 |
| [!UICONTROL **共用對象**] | 顯示目前與誰共用此專案。 |
| [!UICONTROL **上次修改日期**] | 上次修改專案的日期和時間。 |
| [!UICONTROL **上次開啟**] | 識別目前正在查看「專案」頁面的使用者上次開啟專案的日期。 |
| [!UICONTROL **上次使用**] | 透過顯示組織內任何使用者上次開啟專案的日期和時間，協助判斷該專案對組織中的使用者是否有價值。<p>檢視此欄時請考慮以下事項：</p><ul><li>使用情況資訊從 2023 年 9 月開始提供。</li><li>本欄僅適用於系統管理員。</li></ul> |
| [!UICONTROL **專案 ID**] | 可用來為專案偵錯。 |
| [!UICONTROL **最大日期範圍**] | 更長的日期範圍會增加專案複雜性，並可能增加處理和載入時間。 |
| [!UICONTROL **查詢數**] | 專案載入時，向 Analytics 提出的請求總數。 較高的專案查詢數會增加專案複雜性，並可能增加處理和載入時間。 載入專案或傳送已排程專案後才能使用此資料。 |
...
| [!UICONTROL 開啟發行說明] | 開啟最新Adobe CX企業版發行說明的Adobe Analytics區段。 |
...
| &lt; (返回按鈕) | 將您傳回至工作區專案或壣表中最新的登陸頁面設定。 您離開登陸頁面時的頁面設定，會在您返回時保留下來。 |
...
* **追蹤進度：**&#x200B;在選取一段內容後，會出現「**[!UICONTROL 已檢視]**」標記。 此標記可幫助您追蹤自己在學習內容中的進度。 您可以選取&#x200B;**[!UICONTROL 已檢視]**&#x200B;標記以將其從內容中刪除。
...
| 我先前在 Reports &amp; Analytics 中的「我的最愛」是否會移轉過來？ | 否，這些不會移轉過來。 但是，[!UICONTROL 工作區]中任何「我的最愛」會移轉過來。 |
