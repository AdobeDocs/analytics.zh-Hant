---
description: 說明登錄頁面如何將Analysis Workspace和Reports & Analytics整合至Workspace傘下的單一介面和存取點。
title: Adobe Analytics登陸頁面
role: User, Admin
...
Adobe Analytics的登陸頁面會將Analysis Workspace和Reports &amp; Analytics整合在「工作區」傘下的單一介面和存取點中。 它提供專案管理員首頁、更新的報表功能表、現代化的報表和學習區段，協助您更有效率地開始使用。
...
## 存取登錄頁面 {#access-landing}

您登入 Adobe Experience CLoud 和 Analytics 後，可在左下方角落處啟用「[!UICONTROL 全新登陸頁面 - Beta 版]」的切換按鈕。對切換按鈕的存取權是每個組織的使用者專屬權限，而非公司專屬權限。

![登陸](assets/landing.png)

## 功能表結構

![頂端功能表](assets/top-menus.png)

* 頂端Analytics功能表：現在，大部分的報表都位於左側邊欄的[!UICONTROL Reports]功能表中。
* 左邊欄有三個標籤：[!UICONTROL 專案]、[!UICONTROL 報表]和[!UICONTROL 學習]。
...
![全部著陸](assets/landing-all.png)

>[!NOTE]
>
>以下幾項設定會在本工作階段並跨多個工作階段中持續使用 (被記住)。範例：您開啟的標籤、選取的篩選器、選取的欄項，以及欄項排序方向。但是，搜尋結果不會持續存在。
...
| 顯示較少/較多 | 在不顯示和顯示橫幅之間切換：![頂端橫幅](assets/top-banner.png) |
...
| 篩選器圖示 | 您可按標籤、報表套裝、所有者、類型和其他篩選器 (「我的」、「已共用給我」、「我的最愛」和「已核准」) 來篩選 |
| 搜尋列 | 現在的搜尋包含表中的所有欄項。 |
...
![報表標籤](assets/reports-tab.png)

如前面所提，此處只提供先前在 Reports &amp; Analytics 中分類的最常用報告。一小部分很少使用或不再相關的報表未移轉過來。如需詳細資訊，請參閱以下的常見問題。

菜單及其子菜單如下。 如果找不到特定報表，請執行「搜尋頁面」後尋找。

| 功能表項目 | 在此功能表項目下的報表 |
| --- | --- |
| **[!UICONTROL 最受歡迎]** | <ul><li>培訓教學課程 (預存的 Workspace 範本)</li><li>頁面 (哪些是我的主要頁面？)</li><li>頁面檢視 (我產生多少的網頁流量？)</li><li>造訪次數 (我有多少造訪次數？)</li><li>訪客數 (我有多少位訪客？)</li><li>關鍵量度 (我的最重要量度表現如何？)</li><li>網站區段 (我的網站哪些區段產生最多頁面檢視流量？)</li><li>下一頁 (我的訪客前往下一頁是什麼？)</li><li>前一頁 (我的訪客回到前一頁是什麼？)</li><li>行銷活動 (哪些行銷活動在驅動我的關鍵量度？)</li><li>產品 (哪些產品在驅動我的關鍵量度？)</li><li>上次接觸頻道 (哪些特定的上次接觸頻道表現最佳？)</li><li>上次接觸頻道詳細資訊 (哪些特定的上次接觸頻道表現優於其他頻道？)</li><li>收入 (我的收入表現如何？)</li><li>訂單 (我的訂單表現如何？)</li><li>單位 (我售出多少單位？)</li></ul> |
| **[!UICONTROL 參與]** | <ul><li>關鍵量度 (我的最重要量度表現如何？)</li><li>頁面檢視 (我產生多少頁面流量？)</li><li>頁面 (哪些是我的主要頁面？)</li><li>造訪次數 (我有多少造訪次數？)</li><li>訪客數 (我有多少位訪客？)</li><li>每次造訪所花時間 (我的用戶最新造訪時會花多久時間？)</li><li>事件前停留的時間 (我的用戶在成功事件前會停留多久時間？)</li><li>網站區段 (我的網站哪些區段產生最多頁面流量？)</li><li>Web 內容使用 (最常使用且吸引用戶的哪些內容？)</li><li>媒體內容使用 (最常使用且吸引用戶的哪些內容？)</li><li>接下來和先前的頁面流量 (我的訪客接下來/之前走過哪些路徑？)</li><li>流失 (我可以從哪裡看出數位財產的流失？)</li><li>跨裝置分析 (使用 Analysis Workspace 中的跨裝置分析)</li><li>網頁保留率 (我的忠實用戶是哪些人？他們都進行什麼活動？)</li><li>媒體音訊使用量 (音訊使用量有哪些趨勢？最佳量度是哪些？)</li><li>媒體、造訪間隔、頻率、忠誠度 (誰是我的忠實讀者？)</li><li>頁面分析 > 重新載入 (哪些頁面產生最多重新載入？)</li><li>頁面分析 > 每個頁面所花時間 (我的用戶在我的頁面會花多久時間？)</li><li>登入和退出 > 登入頁面 (哪些是我的熱門登入頁面？)</li><li>登入和退出 > 原始登入頁面 (我的用戶最初從哪個面登入？)</li><li>登入和退出 > 單頁存取 (哪些頁面產生最多單頁存取？)</li><li>登入和離開 > 退出頁面 (哪些是我的熱門退出頁面？)</li></ul> |
...
| **[!UICONTROL 贏取]** | <ul><li>行銷頻道 > 首次接觸頻道、首次接觸頻道詳細資料 (首次接觸頻道，以及哪個特定的首次接觸頻道表現最好？)</li><li>行銷頻道 > 首次的上次接觸頻道、首次的上次接觸頻道詳細資料 (哪個上次接觸頻道，以及哪個特定的上次接觸頻道表現最好？)</li><li>行銷活動 > 行銷活動 (哪些行銷活動在驅動我的關鍵量度？)</li><li>行銷活動 > 行銷活動績效 (哪些行銷活動帶來最多收入？)</li><li>行銷活動 > 追蹤程式碼 (哪些行銷活動追蹤程式碼表現最好？)</li><li>[網頁收購](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/build-workspace-project/starter-projects.html?lang=zh-Hant#web)</li><li>[行動收購](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/build-workspace-project/starter-projects.html?lang=en#mobile)</li><li>[Advertising Analytics：付費搜尋](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/build-workspace-project/starter-projects.html?lang=zh-Hant#advertising)</li><li>搜尋關鍵字 - 全部、付費、免費 (哪個搜尋關鍵字和付費/免費搜尋關鍵字可讓我的關鍵量度效果最好？)</li><li>搜尋引擎 - 全部、付費、免費 (哪個搜尋引擎和付費/免費搜尋引擎可讓我的關鍵量度效果最好？)</li><li>全部搜尋頁面排名 (我的用戶從哪個搜尋頁面造訪我的網站？)</li><li>反向連結網域 (哪些網域會增進我的網站流量？)</li><li>原始反向連結網域 (用戶造訪我的網站前會去的第一個網域是哪裡？)</li><li>反向連結 (用戶從哪些網址點進我的網站？)</li><li>反向連結類型 (我的參照 URL 屬於哪個類別？)</li></ul> |
...
* 按一下影片/導覽，可新增「**[!UICONTROL 已檢視]**」標籤。此標籤可幫助您透過學習內容追蹤您的進度。您可以按一下此標記，然後標籤會消失，以防您尚未完成內容。
...
## 登錄頁面常見問題集 {#landing-faq}

| 問題 | 回答 |
| --- | --- |
| 我以前在 Workspace 看到的範本在哪裡了？ | 這些範本分類列在「[!UICONTROL 報表]」標籤下。 |
