...
## 建立資料動態 {#create-data-story}

資料動態是以中央主題或量度為基礎，建置的支援資料點、業務環境和相關量度的集合。

例如，若您專注於網路流量，最重要的量度可能是造訪，但您也可能對新訪客、不重複訪客感興趣，而您可能想要查看依網頁或流量來自何種裝置類型而劃分的資料。 行動計分卡專案中的資料動態可讓您將最重要的量度放在首位，同時透過多張詳細資料投影片將整個動態置於量度後面。

觀看影片，進一步了解如何在Analysis Workspace的行動計分卡專案中建立資料動態。
...
1. 從行動計分卡專案開始。
1. 選取要從中建立動態的圖磚。
   ![建立資料動態](assets/data-story1.png)
   ![建立資料動態圖示](assets/create-data-story.png){width=".50%"}
1. 新增投影片以建立您的資料內容。 預設會產生您的第一張投影片。
若要新增投影片，請將滑鼠暫留在投影片上或按一下投影片，然後從可用選項中選取：
   * 點選+號以建立新的投影片。
   * 點選重複圖示以複製現有投影片。
1. 如果您建立空白幻燈片，請從左側邊欄拖放元件，或選擇版面以自動將圖磚中的資料填入幻燈片。
   ![建立資料動態](assets/data-story2.png)
若要刪除投影片，請點選垃圾桶圖示。

### 自訂資料動態 {#customize-data-story}

資料動態可讓您自訂所有內容，以便共用您想要共用的資訊，並排除您不需要的所有內容。 您可以自訂圖磚和個別投影片，以新增篩選器、顯示劃分、變更版面配置和變更視覺效果。

**自訂圖磚**

1. 點選圖磚。 選定的表徵圖以藍色外框，右側面板顯示表徵圖屬性。
1. 變更標題、圖表類型和其他圖磚選項。
1. 將元件拖曳至圖磚。
   ![建立資料動態](assets/data-story3.png)
將元件（如視覺效果）拖放至圖磚時，元件會套用至所有資料文章投影片。
1. 要僅對標題應用更改，請按住Shift鍵以應用更改。
   ![建立資料動態](assets/data-story4.png)

>[!NOTE]
>投影片會從圖磚繼承元件，但圖磚不會從投影片繼承元件。

**自定義單個幻燈片**

您可以變更資料文章中個別投影片的視覺效果。 例如，您可以將特定幻燈片的水準條更改為環形圖。 您也可以變更版面。 請參閱 [自定義詳細幻燈片](#customize-detail-slide).

### 預覽資料動態 {#preview-data-story}

建立資料動態後，請使用 **預覽** 按鈕來檢視及與資料動態互動，就像您是應用程式使用者。 如需預覽資料動態的相關資訊，請參閱 [預覽計分卡](#preview)

### 在圖磚和投影片之間導覽 {#navigate-tiles-slides}

導覽列會顯示代表每張投影片內容的圖示。 如果您有許多幻燈片，導航欄可讓您輕鬆導航到特定幻燈片。

若要在圖磚和投影片之間移動，請點選導覽列。
![建立資料動態](assets/data-story5.png)
![建立資料動態](assets/data-story-nav.png){width="25%"}

您也可以使用鍵盤上的箭頭，或選取元件並將其維持在畫面左或右以捲動，來來回導覽。
