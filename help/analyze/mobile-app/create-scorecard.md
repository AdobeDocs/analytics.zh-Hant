...
例如，若要將適當的維度 (如本範例中的 **[!DNL Marketing Channel]**) 新增至&#x200B;**[!UICONTROL 不重複訪客]**&#x200B;量度，您可將其拖放至圖磚上。Dimension劃分會顯示在 [!UICONTROL 鑽取] 詳細幻燈片的部分 **[!UICONTROL 屬性]**. 每個圖磚可新增多個維度。
...
按一下計分卡建立程式中的圖磚時，右側邊欄會顯示與該圖磚及其詳細資料投影片相關聯的屬性和特性。 在此邊欄中，您可以提供新 **標題** ，或透過套用區段來設定圖磚。

![「屬性」圖磚](assets/properties_tile.png)

## 查看詳細幻燈片 {#view-detail-slides}

按一下圖磚時，動態快顯視窗會顯示應用程式中執行使用者如何看到詳細資料投影片。 您可以新增維度，以根據您的特定需求劃分資料。 若未套用任何維度，劃分維度將會 **小時** 或 **天數**，視預設日期範圍而定。

劃分會依維度項目字面劃分量度，借此縮小分析範圍。
...
新增至詳細資料投影片的每個維度，都會顯示在應用程式中詳細資料投影片檢視的下拉式清單中。 高階主管使用者可從下拉式清單中選擇選項。

## 自定義詳細幻燈片 {#customize-detail-slide}

自訂詳細資料投影片可讓您更針對您與對象共用的資訊。 您可以修改每張詳細幻燈片的佈局，並添加文本，以便更好地解釋最終用戶在資料中可能看到的內容。 您也可以使用下拉式功能表來變更圖表類型。

![自定義詳細資訊幻燈片](assets/custom-detail-slide.png)

### 更改幻燈片佈局

更改幻燈片佈局以集中顯示最重要的資訊。 例如，您可以變更版面，只顯示圖表或表格。 要更改幻燈片佈局，請選擇預先設計的格式之一。

![幻燈片佈局](assets/layout.png)

您也可以從左側邊欄拖放視覺效果元件至畫布，以變更投影片版面。 每張詳細資料投影片一次只能容納兩個視覺效果。

![幻燈片佈局更改](assets/slide-layout-change.png)

### 將描述性文字新增至投影片

您可以新增文字，以提供關於圖表中所含內容或資料之細微差別的有意義的資訊。

若要將文字新增至詳細資料投影片，請選取顯示 `T` 符號，或從左側邊欄拖放文字視覺效果元件。 新增文字視覺效果或選擇包含文字的投影片版面時，文字編輯器會自動開啟。 文字編輯器提供格式化文字的所有標準選項。 您可以套用段落、標題和子標題等文字樣式，並套用粗體和斜體字型。 您可以對齊文字、新增項目符號和編號清單，以及新增連結。 完成編輯後，選取文字編輯器右上角的最小化按鈕以關閉它。 若要編輯已新增的文字，請選取鉛筆圖示以再次開啟文字編輯器。

![幻燈片佈局更改](assets/add-descriptive-text.png)
